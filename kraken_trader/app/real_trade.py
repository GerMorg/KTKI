...
def D(v):
 ...
 except (InvalidOperation,ValueError,TypeError):raise ValueError('UngÃ¼ltiger Zahlenwert')
...
class RealTradeEngine:
 ...
 def arm(self,phrase):
  ...
  if phrase!='REALHANDEL AKTIVIEREN':raise ValueError('BestÃ¤tigungsphrase stimmt nicht')
 ...
 def submit(self,symbol,side,volume,order_type='limit',limit_price=None,client_order_id=None,approval_token=None,validate_only=True,automation_secret=None):
  ...
  if side not in ('buy','sell') or order_type not in ('limit','market') or volume<=0:raise ValueError('UngÃ¼ltiger Auftrag')
  ...
  if max_volume<=0 or volume>max_volume:raise ValueError('Real-Auftragslimit nicht konfiguriert oder Ã¼berschritten')
  allowed=[x.strip().upper() for x in self.db.value('real_allowed_symbols','').split(',') if x.strip()]
  if allowed and symbol not in allowed:raise PermissionError('Symbol ist nicht fÃ¼r Realhandel freigegeben')
  ...
  if order_type=='limit' and (max_notional<=0 or notional>max_notional):raise ValueError('Real-Auftragswert nicht konfiguriert oder Ã¼berschritten')
  ...
  if live and (not self.enabled() or not (self._armed(approval_token) or automation_ok)):raise PermissionError('Realhandel ist nicht freigegeben oder nicht aktiv bestÃ¤tigt')
  if live:
   ...
   if int(used)>=cap:raise PermissionError('Tageslimit fÃ¼r RealauftrÃ¤ge erreicht')
