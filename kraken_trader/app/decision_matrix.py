...
class DecisionMatrix:
 ...
 def evaluate(self,symbol,action,context,trade_context='PAPER'):
  ...
  add('SIGNAL_CONFIRMED',context.get('confirmation_count',0)>=context.get('confirmation_required',1),f"BestÃ¤tigung {context.get('confirmation_count',0)}/{context.get('confirmation_required',1)}")
  add('MINIMUM_HOLD',context.get('minimum_hold_ok',True),'Mindesthaltedauer erfÃ¼llt' if context.get('minimum_hold_ok',True) else 'Mindesthaltedauer aktiv')
  ...
  add('DAILY_LIMIT',context.get('daily_limit_ok',True),'Tageslimit verfÃ¼gbar' if context.get('daily_limit_ok',True) else 'TÃ¤gliches Umschichtungslimit erreicht')
  add('POSITIVE_AFTER_COSTS',D(context.get('improvement_after_costs'))>0,'Erwarteter Vorteil nach Kosten positiv' if D(context.get('improvement_after_costs'))>0 else 'Kein positiver Vorteil nach vollstÃ¤ndigen Kosten',{'eur':str(context.get('improvement_after_costs',0))})
  ...
  add('DATA_FRESHNESS',context.get('data_fresh',False),'Daten vollstÃ¤ndig und aktuell' if context.get('data_fresh',False) else 'Daten fehlen oder sind veraltet')
  ...
  allowed=all(x['passed'] for x in checks);blocker=next((x['reason'] for x in checks if not x['passed']),'Alle Regeln erfÃ¼llt')
