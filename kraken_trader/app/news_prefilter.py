...
class NewsPrefilter:
 def __init__(self,db,timeout=15):self.db,self.timeout=db,timeout;self.ensure();self.external_ai=ExternalNewsAI(db,{})
 def ensure(self):
 ...
 def _read(self,url,attempts=3):
  headers={'User-Agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 HA-Kraken-Trader/0.1.0-dev.17','Accept':'application/rss+xml,application/xml,application/json,text/xml;q=0.9,*/*;q=0.5'};last=None
 ...
 def collect(self):
  ...
  ai_result=self.external_ai.analyze_pending() if getattr(self,'external_ai',None) else {'status':'DISABLED'};self.db.audit('NEWS_COLLECT',json.dumps({'saved':saved,'errors':errors,'ai':ai_result},ensure_ascii=False),'warning' if errors else 'info');return {'saved':saved,'errors':errors,'ai':ai_result}
 def link_markets(self,markets,limit=500):
  items=self.db.rows("""SELECT n.id,n.title,n.summary,s.weight,(SELECT relevance FROM ai_news_analyses a WHERE a.news_id=n.id AND a.status='VALID' ORDER BY a.created_at DESC LIMIT 1) ai_relevance,(SELECT confidence FROM ai_news_analyses a WHERE a.news_id=n.id AND a.status='VALID' ORDER BY a.created_at DESC LIMIT 1) ai_confidence FROM news_items n JOIN news_sources s ON s.name=n.source_name ORDER BY n.fetched_at DESC LIMIT ?""",(limit,));links=[]
  for m in markets:
   ...
   for item in items:
    ...
    if hits:
     direct=any(x in specific for x in hits);base_rel=float(item['weight'])*(1.0 if direct else .25);ai_rel=float(item.get('ai_relevance') or 0);ai_conf=float(item.get('ai_confidence') or 0);rel=base_rel*(0.75+0.25*ai_rel*ai_conf);links.append((item['id'],symbol,str(rel),('Direkter Marktbezug: ' if direct else 'Kategorietrend: ')+', '.join(hits[:4])+f'; KI-Faktor {ai_rel*ai_conf:.3f}'))
