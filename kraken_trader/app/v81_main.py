...
default and autonomous execution still requires the automation secret, kill
switch clearance, model-health/cost/risk gates and order limits.
"""
import json,os
...
install_real_settings(legacy.db,_options())
...
def v81_health():
 ...
 return {'version':'0.1.0-dev.81','runtime':'v81_main','real_trading_enabled':legacy.db.value('real_trading_enabled','false')=='true','real_kill_switch':legacy.db.value('real_kill_switch','true')=='true','real_balancing_enabled':legacy.db.value('real_balancing_enabled','false')=='true','real_balancing_execute_enabled':legacy.db.value('real_balancing_execute_enabled','false')=='true','automation_secret_configured':bool(legacy.db.value('real_balancing_automation_secret_hash','')),'automation':cfg,'recent_real_runs':controller.latest(20)}
