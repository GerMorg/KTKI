...
class MarketPrefilter:
 ...
 def markets(self):
  raw=self.db.rows("SELECT u.symbol,u.asset_class,u.base_asset,u.quote_asset,u.source_key,u.category primary_category,m.category FROM market_universe u JOIN market_category_members m ON m.symbol=u.symbol AND m.asset_class=u.asset_class JOIN product_categories c ON c.category=m.category AND c.enabled=1 WHERE u.status='online' AND (u.symbol LIKE '%/EUR' OR u.symbol LIKE '%/USD') ORDER BY u.symbol")
  # One market may belong to xstocks and leveraged_spot. prefilter_results is deliberately one row per run and symbol.
  priority={'xstocks':0,'forex':1,'crypto_spot':2,'leveraged_spot':3};by={}
  for row in raw:
   current=by.get(row['symbol'])
   if current is None or priority.get(row['category'],9)<priority.get(current['category'],9):by[row['symbol']]=row
 ...
 def run(self,top=8):
  ...
  for ac,syms in groups.items():
   for block in chunks(syms):
    ...
    except Exception as exc:errors.append({'source':ac,'error':type(exc).__name__})
  ...
  for m in markets:
   ...
   liq=spread_s=mom=0.;quality='VALID' if t else 'NO_TICKER';reasons=[]
   if t:
    bid=float((t.get('b') or [0])[0] or 0);ask=float((t.get('a') or [0])[0] or 0);last=float((t.get('c') or [0])[0] or 0);op=float(t.get('o') or 0);vol=float((t.get('v') or [0,0])[-1] or 0);mid=(bid+ask)/2;sp=(ask-bid)/mid*100 if mid else 999;chg=(last/op-1)*100 if op else 0;turn=max(0,vol*last);spread_s=max(0,35-min(35,sp*35));mom=max(0,min(25,12.5+chg*2));liq=max(0,min(30,math.log10(turn+1)*5));reasons=[f'Spread {sp:.3f} %',f'24h-VerÃƒÂ¤nderung {chg:.2f} %',f'Umsatzindikator {turn:.2f}']
  ...
  for cat in sorted({x['category'] for x in valid}):chosen+=sorted([x for x in valid if x['category']==cat],key=lambda x:x['score'],reverse=True)[:top]
  with self.db.con() as c:
   cur=c.execute('INSERT INTO prefilter_runs VALUES(NULL,?,?,?,?,?,?)',(stamp,'VALID' if valid else 'INCOMPLETE',len(markets),len(chosen),nr['saved'],json.dumps({'errors':errors+nr['errors']},ensure_ascii=False)));rid=cur.lastrowid
   c.executemany('INSERT OR REPLACE INTO prefilter_results VALUES(?,?,?,?,?,?,?,?,?,?)',[(rid,x['symbol'],x['category'],str(x['score']),str(x['liq']),str(x['spread']),str(x['mom']),str(x['news']),x['quality'],json.dumps(x['reasons'],ensure_ascii=False)) for x in rows]);c.execute("UPDATE research_watchlist SET status='STALE'");c.executemany('INSERT OR REPLACE INTO research_watchlist VALUES(?,?,?,?,?,?,?)',[(x['symbol'],x['category'],str(x['score']),'PREFILTERED',stamp,rid,json.dumps(x['reasons'],ensure_ascii=False)) for x in chosen]);c.execute('INSERT INTO watchlist_versions VALUES(NULL,?,?,?,?,?)',(stamp,rid,'CREATED',len(chosen),json.dumps(chosen,ensure_ascii=False)))
  return {'run_id':rid,'markets':len(markets),'valid':len(valid),'candidates':len(chosen),'errors':errors+nr['errors']}
