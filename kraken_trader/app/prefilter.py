...
class MarketPrefilter:
 ...
 def run(self,top=8):
  ...
  for m in markets:
   ...
   if t:
    bid=float((t.get('b') or [0])[0] or 0);ask=float((t.get('a') or [0])[0] or 0);last=float((t.get('c') or [0])[0] or 0);op=float(t.get('o') or 0);vol=float((t.get('v') or [0,0])[-1] or 0);mid=(bid+ask)/2;sp=(ask-bid)/mid*100 if mid else 999;chg=(last/op-1)*100 if op else 0;turn=max(0,vol*last);spread_s=max(0,35-min(35,sp*35));mom=max(0,min(25,12.5+chg*2));liq=max(0,min(30,math.log10(turn+1)*5));reasons+=['AusfÃ¼hrungskosten gesamt '+str(round(float(m['execution_costs']['total_rate'])*100,4))+' %',f'Spread {sp:.3f} %',f'24h-VerÃ¤nderung {chg:.2f} %',f'Umsatzindikator {turn:.2f}']
   if not t:reasons.append('Markt wurde von Kraken gemeldet, Ticker war im Vorfilter aber nicht verfÃ¼gbar; Kandidat bleibt fÃ¼r DetailprÃ¼fung erhalten')
