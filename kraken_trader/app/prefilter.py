...
class MarketPrefilter:
 ...
 def run(self,top=8):
  ...
  rows=[];stamp=now()
  for m in markets:
   symbol=m['symbol'];t=tickers.get(m['source_key']) or tickers.get(symbol)
   if t is None:
   ...
   if t:
    bid=float((t.get('b') or [0])[0] or 0);ask=float((t.get('a') or [0])[0] or 0);last=float((t.get('c') or [0])[0] or 0);op=float(t.get('o') or 0);vol=float((t.get('v') or [0,0])[-1] or 0);mid=(bid+ask)/2;sp=(ask-bid)/mid*100 if mid else 999;chg=(last/op-1)*100 if op else 0;turn=max(0,vol*last);spread_s=max(0,35-min(35,sp*35));mom=max(0,min(25,12.5+chg*2));liq=max(0,min(30,math.log10(turn+1)*5));reasons=[f'Spread {sp:.3f} %',f'24h-VerÃ¤nderung {chg:.2f} %',f'Umsatzindikator {turn:.2f}']
  ...
  with self.db.con() as c:
   ...
   c.executemany('INSERT INTO prefilter_results VALUES(?,?,?,?,?,?,?,?,?,?)',[(rid,x['symbol'],x['category'],str(x['score']),str(x['liq']),str(x['spread']),str(x['mom']),str(x['news']),x['quality'],json.dumps(x['reasons'],ensure_ascii=False)) for x in rows]);c.execute("UPDATE research_watchlist SET status='STALE'");c.executemany('INSERT OR REPLACE INTO research_watchlist VALUES(?,?,?,?,?,?,?)',[(x['symbol'],x['category'],str(x['score']),'PREFILTERED',stamp,rid,json.dumps(x['reasons'],ensure_ascii=False)) for x in chosen]);c.execute('INSERT INTO watchlist_versions VALUES(NULL,?,?,?,?,?)',(stamp,rid,'CREATED',len(chosen),json.dumps(chosen,ensure_ascii=False)))
