...
class MarketPrefilter:
 ...
 def run(self,top=8):
  ...
  for m in markets:
   ...
   seen.add(symbol);t=tickers.get(m.get('source_key')) or tickers.get(symbol)
   if t is None:
   ...
   if isinstance(t,dict):
    bid=float((t.get('b') or [0])[0] or 0);ask=float((t.get('a') or [0])[0] or 0);last=float((t.get('c') or [0])[0] or 0);op=float(t.get('o') or 0);vol=float((t.get('v') or [0,0])[-1] or 0);mid=(bid+ask)/2;sp=(ask-bid)/mid*100 if mid else 999;chg=(last/op-1)*100 if op else 0;turn=max(0,vol*last);spread_s=max(0,35-min(35,sp*35));mom=max(0,min(25,12.5+chg*2));liq=max(0,min(30,math.log10(turn+1)*5));costs=m.get('execution_costs') or {};reasons+=['Ausführungskosten gesamt '+str(round(float(costs.get('total_rate',999))*100,4))+' %',f'Spread {sp:.3f} %',f'24h-Veränderung {chg:.2f} %',f'Umsatzindikator {turn:.2f}']
   if not isinstance(t,dict):reasons.append('Markt wurde von Kraken gemeldet, Ticker war im Vorfilter aber nicht verfügbar; Kandidat bleibt für Detailprüfung erhalten')
  ...
  for cat in sorted({x['category'] for x in rows}):
   pool=sorted([x for x in rows if x['category']==cat],key=lambda x:(x['quality']=='VALID',x['score']),reverse=True);chosen+=pool[:top]
  with self.db.con() as c:
   cur=c.execute('INSERT INTO prefilter_runs VALUES(NULL,?,?,?,?,?,?)',(stamp,'VALID' if valid else ('DEGRADED' if rows else 'INCOMPLETE'),len(markets),len(chosen),int(nr.get('saved',0) or 0),json.dumps({'errors':errors+list(nr.get('errors',[]) or [])},ensure_ascii=False)));rid=cur.lastrowid
  ...
  return {'run_id':rid,'markets':len(markets),'valid':len(valid),'pending_ticker':sum(x['quality']=='PENDING_TICKER' for x in rows),'candidates':len(chosen),'errors':errors+list(nr.get('errors',[]) or [])}
