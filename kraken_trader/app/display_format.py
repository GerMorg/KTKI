...
def display_number(value, decimals=None):
    ...
    absolute=abs(number)
    if decimals is None:
        if absolute==0: places=0
        elif absolute>=Decimal('1'): places=2
        elif absolute>=Decimal('0.1'): places=4
        elif absolute>=Decimal('0.01'): places=6
        elif absolute>=Decimal('0.001'): places=7
        else: places=8
    else: places=max(0,int(decimals))
...
def display_tree(value):
    if isinstance(value,dict): return {key:display_tree(item) for key,item in value.items()}
    if isinstance(value,list): return [display_tree(item) for item in value]
    if isinstance(value,tuple): return tuple(display_tree(item) for item in value)
    if value is None or isinstance(value,bool): return value
    if isinstance(value,float): return DisplayFloat(value)
    if isinstance(value,Decimal): return DisplayFloat(value)
    if isinstance(value,str) and _NUMERIC_TEXT.fullmatch(value.strip()):
        text=value.strip()
        if '.' in text and len(text.rsplit('.',1)[1])>2: return DisplayNumberText(text)
    return value
