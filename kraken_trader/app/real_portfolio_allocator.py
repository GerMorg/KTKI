...
class RealPortfolioAllocator:
 ...
 def _alternatives(self,symbol):
  rows=self.db.rows('SELECT canonical_id FROM market_universe WHERE symbol=? LIMIT 1',(symbol,));cid=rows[0]['canonical_id'] if rows else symbol
  alts=self.db.rows('SELECT symbol,asset_class,category,base_asset,quote_asset,source_key,ordermin,costmin FROM market_universe WHERE canonical_id=? AND quote_asset IN (\'EUR\',\'USD\')',(cid,))
  return alts or self.db.rows('SELECT symbol,asset_class,category,base_asset,quote_asset,source_key,ordermin,costmin FROM market_universe WHERE symbol=?',(symbol,))
 def _candidates(self,cfg):
  tickers=self._tickers();out=[]
  rows=self.db.rows("SELECT s.symbol,s.score,s.volatility_pct,s.signal,s.quality FROM scanner_results s WHERE s.quality='VALID' AND s.signal='BUY' AND CAST(s.score AS REAL)>=? ORDER BY CAST(s.score AS REAL) DESC",(str(cfg['minimum_score']),))
  for row in rows:
   if cfg['allowed_symbols'] and row['symbol'].upper() not in cfg['allowed_symbols']:continue
   alts=self._alternatives(row['symbol']);family=family_for_category((self.db.rows('SELECT category FROM market_universe WHERE symbol=? LIMIT 1',(row['symbol'],)) or [{'category':'crypto_spot'}])[0]['category']);version,params=active_profile(self.db,family);selected,route=choose_route(alts,tickers,100, self.db.value('real_fee_bps',self.db.value('paper_fee_bps','40')),self.db.value('real_fx_fee_bps',self.db.value('paper_fx_fee_bps','10')),self.db.value('real_slippage_bps',self.db.value('paper_slippage_bps','10')),'buy')
   if not selected:continue
   out.append({'symbol':row['symbol'],'score':row['score'],'volatility_pct':row['volatility_pct'],'roundtrip_cost_pct':str(route['selected']['total_cost_pct'] if route.get('selected') else 999),'buy_threshold':params.get('buy_threshold',62),'family':family,'model_version':version,'selected':selected,'route':route})
  return out
 def _base_exposure(self,asset,current):return D(current.get(self._asset(asset),0))
 def _volume(self,symbol,trade_eur,side,route):
  price=D((self.db.rows('SELECT ask,bid,last FROM live_prices WHERE symbol=? LIMIT 1',(symbol,)) or [{'ask':0,'bid':0,'last':0}])[0].get('ask' if side=='buy' else 'bid') or (self.db.rows('SELECT last FROM live_prices WHERE symbol=? LIMIT 1',(symbol,)) or [{'last':0}])[0]['last']);
 ...
 def run(self,automatic=False,approval_token=None):
  ...
  try:
   ...
   current,total=self._current_eur();payload={k:(str(v) if isinstance(v,Decimal) else v) for k,v in cfg.items()};tickers=self._tickers();health=ModelHealth(self.db);health_by_family={}
   for family in ('crypto_spot','xstocks','forex'):health_by_family[family]=health.evaluate(family)
   candidates=self._candidates(cfg)
   target_rows=[]
   for c in candidates:
    target_rows.append({'symbol':c['symbol'],'score':c['score'],'volatility_pct':c['volatility_pct'],'roundtrip_cost_pct':c['roundtrip_cost_pct'],'buy_threshold':c['buy_threshold']})
   targets=build_targets(target_rows,total,cfg['cash_reserve_pct'],cfg['max_position_pct'],cfg['minimum_score'],cfg['min_trade_eur'])
   with self.db.con() as con:cur=con.execute('INSERT INTO real_allocation_runs(created_at,status,automatic,settings_json,details_json) VALUES(?,?,?,?,?)',(now(),'RUNNING',1 if automatic else 0,json.dumps(payload,sort_keys=True),'{}'));rid=cur.lastrowid
   ...
   for target in targets[:min(cfg['max_actions_per_run'],room)]:
    symbol=target['symbol'];asset=self._asset(symbol.split('/')[0]);present=self._base_exposure(asset,current);target_eur=D(target['target_exposure_eur']);diff=target_eur-present
    ...
    if not selected:continue
    family=next((x['family'] for x in candidates if x['symbol']==symbol),'crypto_spot');h=health_by_family.get(family,{'status':'NOT_READY'});edge=health.expected_edge_pct(family,24);cost_pct=D(route['selected']['total_cost_pct'] if route.get('selected') else 999);edge_after_cost=D(edge or 0)-cost_pct
    quote=str(selected.get('quote_asset') or 'EUR').upper();usd_balance=sum((D(x['balance']) for x in self.balances() if self._asset(x['asset'])=='USD'),D(0));fx_ok=True;funding={'required':False}
    ...
    rowmeta=self.db.rows('SELECT ordermin,costmin FROM market_universe WHERE symbol=? LIMIT 1',(selected['symbol'],));meta=rowmeta[0] if rowmeta else {};order_ok=(not meta.get('ordermin') or volume>=D(meta['ordermin'])) and (not meta.get('costmin') or volume*price>=D(meta['costmin']))
    risk_ok=target_eur<=total*cfg['max_position_pct']/100 and target_eur<=total*(1-cfg['cash_reserve_pct']/100)
    ctx={'canonical_id':symbol,'confirmation_count':1,'confirmation_required':1,'minimum_hold_ok':True,'cooldown_ok':True,'daily_limit_ok':True,'improvement_after_costs':str(max(D(0),edge_after_cost)*trade_eur/100),'tax_loss_ok':True,'data_fresh':True,'model_health_ok':h.get('status')=='READY','model_health_details':h,'route_cost_ok':route.get('status')=='VALID','route_cost_details':route,'quote_funding_ok':fx_ok and (not funding['required'] or cfg['dry_run']),'quote_funding_details':funding,'portfolio_risk_ok':risk_ok,'portfolio_risk_details':{'target_eur':str(target_eur),'total_eur':str(total)},'order_constraints_ok':order_ok,'order_constraints_details':{'volume':str(volume),'price':str(price),'ordermin':meta.get('ordermin'),'costmin':meta.get('costmin')},'real_trading_enabled':self.trade_engine.enabled(),'real_kill_switch_clear':self.db.value('real_kill_switch','true').lower()!='true','real_limits_ok':trade_eur<=cfg['max_trade_eur'],'real_balance_ok':True}
    ...
    if execute and funding['required']:
     funding_result=self.trade_engine.convert_eur_to_usd(funding['needed_eur'],None,approval_token,False)
     funding['result']=funding_result
     status='FUNDING_VALIDATED' if funding_result['status']=='VALIDATED' else 'FUNDING_FAILED'
    elif execute:
     secret=self.db.value('real_balancing_automation_secret','');volume,price=self._volume(selected['symbol'],trade_eur,side,route);result=self.trade_engine.submit(selected['symbol'],side,str(volume),'limit',str(price),secrets.token_hex(16),approval_token,False,secret);status=result['status'];intent=result.get('client_order_id')
    elif decision['allowed']:status='DRY_RUN' if cfg['dry_run'] or automatic else 'PROPOSED'
    with self.db.con() as con:con.execute('INSERT INTO real_allocation_actions(run_id,created_at,symbol,side,current_eur,target_eur,difference_eur,status,decision_json,order_intent_id,error) VALUES(?,?,?,?,?,?,?,?,?,?,?)',(rid,now(),symbol,side,str(present),str(target_eur),str(diff),status,json.dumps({'decision':decision,'route':route,'funding':funding,'model_health':h},sort_keys=True),intent,error))
   ...
   final='COMPLETED' if all(x['status'] not in ('FAILED','FUNDING_FAILED') for x in actions) else 'PARTIAL'
   with self.db.con() as con:con.execute('UPDATE real_allocation_runs SET finished_at=?,status=?,details_json=? WHERE id=?',(now(),final,json.dumps({'total_eur':str(total),'actions':actions,'model_health':health_by_family},sort_keys=True),rid))
   self.db.audit('REAL_BALANCING_RUN',json.dumps({'run_id':rid,'status':final,'automatic':automatic,'actions':len(actions)}),'warning' if automatic else 'info','REAL');return {'status':final,'run_id':rid,'total_eur':str(total),'actions':actions,'settings':payload,'model_health':health_by_family}
