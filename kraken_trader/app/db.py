...
class _LockedConnection:
 def __init__(self,raw,lock):self._raw=raw;self._lock=lock
 def __enter__(self):self._lock.acquire();return self._raw.__enter__()
 def __exit__(self,*args):
  try:return self._raw.__exit__(*args)
...
class DB:
 ...
 def con(self):
  c=sqlite3.connect(self.path,timeout=30.0,check_same_thread=False);c.row_factory=sqlite3.Row;c.execute('PRAGMA busy_timeout=30000');c.execute('PRAGMA foreign_keys=ON');return _LockedConnection(c,self._lock)
 ...
 def audit(self,e,d='',level='info',context='SYSTEM'):
  ...
  def run():
   with self.con() as c:c.execute('INSERT INTO audit(created_at,event,level,details,trade_context) VALUES(?,?,?,?,?)',(now(),e,level,d,context))
  return self._retry(run)
 def replace_balances(self,items):
  def run():
   with self.con() as c:c.execute('DELETE FROM balances');c.executemany('INSERT INTO balances VALUES(?,?,?)',[(k,str(v),now()) for k,v in items.items()])
  return self._retry(run)
