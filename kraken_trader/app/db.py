...
class DB:
 ...
 def init(self,start=1000):
  with self.con() as c:
   c.executescript("""CREATE TABLE IF NOT EXISTS settings(key TEXT PRIMARY KEY,value TEXT NOT NULL);CREATE TABLE IF NOT EXISTS balances(asset TEXT PRIMARY KEY,amount TEXT NOT NULL,updated_at TEXT NOT NULL);CREATE TABLE IF NOT EXISTS paper_balances(asset TEXT PRIMARY KEY,amount TEXT NOT NULL,updated_at TEXT NOT NULL);CREATE TABLE IF NOT EXISTS ledger(id TEXT PRIMARY KEY,payload TEXT NOT NULL,occurred_at REAL,imported_at TEXT NOT NULL);CREATE TABLE IF NOT EXISTS audit(id INTEGER PRIMARY KEY AUTOINCREMENT,created_at TEXT NOT NULL,event TEXT NOT NULL,level TEXT NOT NULL,details TEXT NOT NULL);CREATE TABLE IF NOT EXISTS allowlist(symbol TEXT PRIMARY KEY,enabled INTEGER NOT NULL);CREATE TABLE IF NOT EXISTS portfolio_assets(asset TEXT PRIMARY KEY,display_name TEXT,amount TEXT NOT NULL,eur_price TEXT,eur_value TEXT,classification TEXT NOT NULL,ever_held INTEGER NOT NULL,updated_at TEXT NOT NULL);CREATE TABLE IF NOT EXISTS portfolio_snapshots(id INTEGER PRIMARY KEY AUTOINCREMENT,created_at TEXT NOT NULL,total_eur TEXT,priced_asset_count INTEGER NOT NULL,unpriced_asset_count INTEGER NOT NULL,quality TEXT NOT NULL);CREATE TABLE IF NOT EXISTS portfolio_snapshot_items(snapshot_id INTEGER NOT NULL,asset TEXT NOT NULL,amount TEXT NOT NULL,eur_price TEXT,eur_value TEXT,classification TEXT NOT NULL,PRIMARY KEY(snapshot_id,asset));CREATE TABLE IF NOT EXISTS live_prices(symbol TEXT PRIMARY KEY,last TEXT NOT NULL,bid TEXT,ask TEXT,change_pct TEXT,received_at TEXT NOT NULL);CREATE TABLE IF NOT EXISTS stream_state(id INTEGER PRIMARY KEY CHECK(id=1),state TEXT NOT NULL,system_status TEXT,connection_id TEXT,last_message_at TEXT,last_error TEXT,updated_at TEXT NOT NULL);""")
   c.execute("INSERT OR IGNORE INTO stream_state VALUES(1,'STOPPED',NULL,NULL,NULL,NULL,?)",(now(),));c.execute("INSERT OR IGNORE INTO settings VALUES('automation_enabled','false')");c.execute("INSERT OR IGNORE INTO settings VALUES('kraken_status','not_checked')");c.execute("INSERT OR IGNORE INTO settings VALUES('websocket_status','not_checked')");c.execute("INSERT OR IGNORE INTO paper_balances VALUES('EUR',?,?)",(str(start),now()))
 def rows(self,q,p=()):
