"""v73 runtime: serialized SQLite access and defensive payload boundaries."""
import threading
import time
...
def _paper_execute_v73(self, symbol, side, gross, reason, decision):
    return _original_paper_execute(self, symbol, side, gross, reason, as_mapping(decision, {'action': side}))

def _allocator_plans_v73(self, total):
    return as_mapping_list(_original_allocator_plans(self, total))

def _decision_evaluate_v73(self, symbol, action, context, trade_context='PAPER'):
    return _original_decision_evaluate(self, symbol, action, as_mapping(context, {}), trade_context)

def _router_find_v73(tickers, *keys):
    return _original_router_find(as_mapping(tickers, {}), *keys)

PaperEngine.execute = _paper_execute_v73
PortfolioAllocator.plans = _allocator_plans_v73
DecisionMatrix.evaluate = _decision_evaluate_v73
execution_router_module._find = _router_find_v73
...
def start_pipeline_v73(*args, **kwargs):
    result = as_mapping(_original_pipeline_start(*args, **kwargs))
    if result.get('job_id'):
        threading.Thread(target=_record_finished_analysis,args=(int(result['job_id']),),daemon=True,name='research-monitor-v73').start()
    return result

controller.pipeline.start = start_pipeline_v73
...
def v73_health():
    ...
    return jsonify({'version':'0.1.0-dev.73','runtime':'v73_main','sqlite_journal_mode':'WAL','sqlite_busy_timeout_ms':30000,'latest_research':as_mapping(job,{}) if job else None,'recent_analysis_failures':[x for x in latest if str(x.get('subsystem')).lower()=='analysis' and str(x.get('status')).upper()=='FAILED'][:10],'automation_master_enabled':str(cfg.get('automation_master_enabled','')).lower()=='true'})
