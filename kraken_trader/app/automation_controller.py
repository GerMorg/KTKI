...
def as_mapping(value):
 ...
 if isinstance(value,list): return {'status':'COMPLETED','items':value}
 if value is None: return {'status':'COMPLETED'}
...
class AutomationController:
 def __init__(self,db,pipeline,news_prefilter,controlled_learning,news_learning,run_paper_cycle,real_allocator):
  self.db=db;self.pipeline=pipeline;self.news_prefilter=news_prefilter;self.controlled_learning=controlled_learning;self.news_learning=news_learning;self.run_paper_cycle=run_paper_cycle;self.real_allocator=real_allocator;self.lock=threading.RLock();self.stop_event=threading.Event();self.thread=None;self.ensure()
 def ensure(self):
 ...
 def _sync(self,s):
  self.db.set('automation_enabled','true' if self.boolean(s['automation_paper_enabled']) else 'false');self.db.set('real_balancing_enabled','true' if self.boolean(s['automation_real_enabled']) else 'false');self.db.set('real_balancing_execute_enabled','true' if self.boolean(s['automation_real_execute_enabled']) else 'false');self.db.set('real_balancing_dry_run','false' if self.boolean(s['automation_real_execute_enabled']) else 'true')
 def _approve(self):
  out=[]
 ...
 def run_learning(self,automatic=True,auto_approve=False):
  result={'status':'COMPLETED','strategy':self.controlled_learning.propose_all(automatic=automatic),'news':self.news_learning.propose(automatic=automatic)}
 ...
 def start_background(self):
  with self.lock:
   ...
   self.stop_event.clear();self.thread=threading.Thread(target=self._loop,daemon=True,name='automation-controller');self.thread.start();return self.thread
 def _loop(self):
