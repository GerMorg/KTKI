...
# Replace the root view without creating a second Flask rule.
...
def v78_health():
    ...
    payload = {
        ...
        'research_details': json.loads(research['details_json']) if research and research.get('details_json') else {},
...
def _repair_analysis():
    """Run a bounded repair pass so a degraded historical database becomes usable again."""
    try:
    ...
    try:
        candidates = legacy.prefilter.candidates()
        if candidates:
    ...
    return len(_safe(lambda: legacy.prefilter.candidates(), []) or [])


@app.post('/v78-analysis/repair')
def v78_analysis_repair():
    count = _repair_analysis()
    return {'status': 'COMPLETED', 'candidate_count': count}
