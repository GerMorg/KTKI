...
def v78_health():
    ...
    if research and research.get('details_json'):
        try: details=json.loads(research['details_json'])
        except Exception: details={'parse_error':True}
    return {'version':'0.1.0-dev.78','runtime':'v78_main','latest_research':research,'research_details':details,'watchlist_count':len(watchlist),'watchlist_preview':watchlist,'portfolio':portfolio[0] if portfolio else None,'public_websocket':public,'private_websocket':private,'dataflow_status':_safe(lambda: legacy.db.value('v77_dataflow_status','STARTING'),'STARTING')}

@app.post('/v78-analysis/repair')
def v78_analysis_repair():
    try: legacy.universe.sync()
    except Exception: pass
    try: legacy.prefilter.run(int(float(legacy.db.value('analysis_max_symbols','20'))))
    except Exception as exc: legacy.db.audit('V78_PREFILTER_REPAIR_FAILED',type(exc).__name__+': '+str(exc)[:300],'warning')
    candidates=_safe(lambda: legacy.prefilter.candidates(),[]) or []
    if candidates:
        try: legacy.scanner.run(candidates,60,limit=len(candidates),delay_seconds=min(float(legacy.db.value('scanner_delay_seconds','1.05')),0.35))
        except Exception as exc: legacy.db.audit('V78_SCANNER_REPAIR_FAILED',type(exc).__name__+': '+str(exc)[:300],'warning')
