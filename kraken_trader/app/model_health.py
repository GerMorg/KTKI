...
class ModelHealth:
 ...
 def expected_edge_pct(self,family,horizon=24,after_costs=False):
  health=self.evaluate(family);item=health['horizons'].get(str(horizon),{});return item.get('expected_up_edge_after_costs_pct' if after_costs else 'expected_up_edge_raw_pct')
