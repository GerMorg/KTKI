"""Model-health checks used before autonomous real decisions."""
import json
...
class ModelHealth:
 ...
 def evaluate(self,family,min_samples=20,min_net_return_pct=0.0,max_drawdown_pct=-25.0):
  ...
  for horizon in self.REQUIRED_HORIZONS:
   subset=[r for r in rows if int(r['horizon_hours'])==horizon];cost_adjusted=[]
   for r in subset:
    try:features=json.loads(r.get('features_json') or '{}')
    except Exception:features={}
    cost=float(features.get('estimated_roundtrip_cost_pct') or 0);direction=str(r.get('direction') or 'FLAT');actual=float(r.get('actual_return_pct') or 0)
    strategy=actual-cost if direction=='UP' else 0.0
   ...
   details['horizons'][str(horizon)]={'samples':n,'hit_rate':hit/n if n else None,'model_net_return_pct':model_net,'no_position_return_pct':0.0,'buy_hold_return_sum_pct':buy_hold,'excess_vs_no_position_pct':model_net,'max_drawdown_pct':dd}
