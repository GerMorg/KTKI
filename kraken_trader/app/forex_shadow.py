...
class ForexShadow:
 ...
 def _currency_strength(self,currency):
  ...
  for raw in rows:
   ...
   vals.append(v if str(x.get('symbol','')).startswith(currency+'/') else -v)
  return sum(vals)/len(vals) if vals else 0.0
 @staticmethod
 def _pair_parts(symbol):
  parts=[part for part in str(symbol or '').split('/') if part]
  return (parts[0] if parts else '',parts[1] if len(parts)>1 else '')
 def score(self,symbol,active):
  active=as_mapping(active,{})
  base,quote=self._pair_parts(symbol);reasons=json.loads(active.get('reasons_json') or '[]') if isinstance(active.get('reasons_json'),str) else [];momentum=float(active.get('momentum_pct') or 0);trend=float(active.get('trend_pct') or 0);vol=float(active.get('volatility_pct') or 0);spread=float(active.get('spread_pct') or 999)
  bs=self._currency_strength(base);qs=self._currency_strength(quote);relative=bs-qs;risk_regime=max(-1,min(1,(SAFE.get(base,0)-SAFE.get(quote,0))));horizons={'short':momentum,'medium':trend};out=[]
  ...
  for entry in horizons.items():
   horizon,hvalue=entry[0],entry[1];score=50+max(-18,min(18,hvalue*(4 if horizon=='short' else 8)))+max(-15,min(15,relative*3))+risk_regime*4-max(0,min(16,vol))-max(0,min(22,spread*28))+min(8,news_relevance);score=max(0,min(100,score));signal='BUY' if score>=65 and spread<=.7 else ('AVOID' if score<35 or spread>1.3 else 'HOLD');quality='VALID' if active.get('quality')=='VALID' and spread<999 else 'INCOMPLETE'
 ...
 def run(self,symbols=None):
  query="SELECT s.* FROM scanner_results s JOIN market_universe u ON u.symbol=s.symbol WHERE u.category='forex'";rows=self.db.rows(query);wanted=set(symbols or []);results=[]
  for raw in rows:
   row=as_mapping(raw,{})
   symbol=row.get('symbol')
   if wanted and symbol not in wanted:continue
   results.extend([dict(x,symbol=symbol) for x in self.score(symbol,row)])
