...
def current_market_batch_v70():
    """Read the market batch from the actual legacy core where it is defined."""
    try:
    ...
    if not isinstance(result, (tuple, list)):
        legacy.db.audit('PAPER_MARKET_BATCH_INVALID', type(result).__name__, 'error')
        return []
    return list(dict.fromkeys(str(x) for x in result if x))
...
# Normalize all scheduler-facing return values so AutomationControllerV67 can use .get()
# safely even when a legacy subsystem returns a list or None.
...
controller.pipeline.start = lambda *a, **k: as_mapping(_original_pipeline_start(*a, **k))
controller.news_prefilter.collect = lambda *a, **k: as_mapping(_original_news_collect(*a, **k))
controller.real_allocator.run = lambda *a, **k: as_mapping(_original_real_run(*a, **k))
...
def chart_v70(values):
    ...
    left, right, top, bottom = 82, 26, 40, 58
    ...
    ys = [top, top+plot_h/3, top+2*plot_h/3, baseline]
    grid = ''.join(f'<line x1="{left}" y1="{y:.1f}" x2="{width-right}" y2="{y:.1f}" class="chart-gridline"/>' for y in ys)
    ylabels = ''.join(f'<text x="{left-12}" y="{y+4:.1f}" text-anchor="end" class="chart-axis-label">{v:,.2f} €</text>' for y,v in [(ys[0],hi),(ys[1],hi-(hi-lo)/3),(ys[2],hi-2*(hi-lo)/3),(ys[3],lo)])
    label_indices = sorted(set([0, (len(vals)-1)//3, 2*(len(vals)-1)//3, len(vals)-1]))
    xlabels = ''.join(f'<text x="{points[i][0]:.1f}" y="{height-20}" text-anchor="middle" class="chart-axis-label">{i+1}</text>' for i in label_indices)
    return f'<svg viewBox="0 0 {width} {height}" class="chart" role="img" aria-label="Portfolio-Wertentwicklung"><rect x="0" y="0" width="{width}" height="{height}" rx="14" class="chart-bg"/>{grid}{ylabels}{xlabels}<polygon points="{area}" class="chart-area"/><polyline points="{line}" class="chart-line" fill="none"/><circle cx="{points[-1][0]:.1f}" cy="{points[-1][1]:.1f}" r="6" class="chart-dot"/><text x="{left}" y="22" class="chart-title">Wertentwicklung</text><text x="{width-right}" y="22" text-anchor="end" class="chart-value">Aktuell {vals[-1]:,.2f} €</text></svg>'
