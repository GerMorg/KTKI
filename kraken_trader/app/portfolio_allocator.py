...
class PortfolioAllocator:
 ...
 def confidence(self,scan):
  ...
  score=D(scan.get('score'))/100;vol=D(scan.get('volatility_pct'));spread=D(scan.get('spread_pct'))
  return max(D(0),min(D(1),score-min(D('.35'),vol/100)-min(D('.20'),spread/5)))
 def leverage(self,symbol,confidence):
  ...
  cap=int(D(self.db.value('paper_max_leverage','3')));allowed=[x for x in levels if x<=cap]
 ...
 def target(self,symbol,scan,total):
  conf=self.confidence(scan);vol=max(D('.5'),D((scan or {}).get('volatility_pct')));floor=D(self.db.value('paper_min_position_pct','2'))/100;cap=D(self.db.value('paper_max_position_pct','10'))/100
  pct=min(cap,max(D(0),(conf*conf)/(D(1)+vol/10)*cap*D('1.8')))
  if pct<floor:pct=D(0)
  return {'symbol':symbol,'confidence':str(conf),'target_pct':str(pct),'target_exposure_eur':str(total*pct),'leverage':self.leverage(symbol,conf),'volatility_pct':str(vol)}
