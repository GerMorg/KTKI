...
# Harden the exact path used by the automatic Paper cycle. The v67 normalization
# covers the execution router, but price refresh still consumed raw Kraken payloads.
_original_refresh_allowed_prices = base.refresh_allowed_prices

def refresh_allowed_prices_v69():
    # The stable v67 implementation is reused, but malformed/list ticker payloads
    # must never reach item.get(). Reimplement only the affected extraction boundary.
...
def run_paper_cycle_v69():
    ...
    if isinstance(result, list):
        # Paper engine responses are expected as a mapping for automation records.
        return {'status': 'COMPLETED', 'actions': result}
    return result

base.controller.run_paper_cycle = run_paper_cycle_v69

# Make the v68 tax GUI a first-class process page.
