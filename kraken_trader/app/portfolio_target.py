...
The scanner score is treated as conviction/ranking, never as a probability.
"""
...
def build_targets(candidates,total_eur,cash_reserve_pct=20,max_position_pct=10,buy_threshold=62,min_target_eur=20):
 ...
 for row in candidates:
  score=D(row.get('score'));vol=max(D('0.25'),abs(D(row.get('volatility_pct') or 0)));threshold=D(row.get('buy_threshold') or buy_threshold);conviction=max(D(0),score-threshold)/vol
 ...
 for _ in range(len(targets)+1):
  used=sum(D(x['target_exposure_eur']) for x in targets);left=budget-used
  uncapped=[x for x in targets if D(x['target_exposure_eur'])<budget*cap-D('0.00000001')]
