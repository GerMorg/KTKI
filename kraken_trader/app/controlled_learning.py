...
class ControlledLearning:
    ...
    def _migrate_legacy_xstocks(self):
        ...
        if not rows or not current or int(current.get('version') or 1) != 1 or current.get('source') != 'DEFAULT':
            return
        ...
        with self.db.con() as c:
            c.execute('UPDATE parameter_family_versions SET parameters_json=?,source=?,reason=? WHERE id=?',
                      (json.dumps(params, sort_keys=True), 'LEGACY_MIGRATION',
                       'Einmalige Übernahme der vorhandenen xStock-Parameter', current['id']))
    ...
    def family_overview(self):
        ...
        for family in FAMILIES:
            ...
            counts = {row['status']: row['n'] for row in self.db.rows('SELECT status,COUNT(*) AS n FROM learning_candidates WHERE family=? GROUP BY status', (family,))}
            latest = self.db.rows('SELECT id,status,created_at,decided_at FROM learning_candidates WHERE family=? ORDER BY id DESC LIMIT 1', (family,))
            result.append({'family': family, 'active_version': active['version'] if active else None,
                           'pending_count': int(counts.get('PENDING', 0)), 'approved_count': int(counts.get('APPROVED', 0)),
                           'rejected_count': int(counts.get('REJECTED', 0)), 'latest_candidate_id': latest[0]['id'] if latest else None,
                           'latest_status': latest[0]['status'] if latest else 'NONE', 'latest_created_at': latest[0]['created_at'] if latest else None,
    ...
    def gate_policy(self):
        def number(key, default, cast=float):
            try: return cast(float(self.db.value(key, str(default))))
    ...
    def _gate_results(self, metrics, accuracy_improvement, minimum_accuracy_improvement, policy=None):
        policy = policy or self.gate_policy(); by_horizon = {int(x['horizon_hours']): x for x in metrics}
        results = [{'gate': 'ACCURACY_IMPROVEMENT', 'horizon_hours': None, 'passed': accuracy_improvement >= minimum_accuracy_improvement,
                    'actual': accuracy_improvement, 'required': minimum_accuracy_improvement}]
        for horizon in policy['required_horizons']:
            metric = by_horizon.get(int(horizon)); results.append({'gate': 'HORIZON_PRESENT', 'horizon_hours': horizon,
                'passed': metric is not None, 'actual': metric is not None, 'required': True})
            if metric is None: continue
            sample_count = int(metric['sample_count']); coverage = float(metric['candidate_coverage']); net_improvement = float(metric['net_return_improvement'])
            candidate_dd = float(metric['candidate_max_drawdown']); active_dd = float(metric['active_max_drawdown'])
            checks = (('MINIMUM_HORIZON_SAMPLE', sample_count >= policy['minimum_horizon_samples'], sample_count, policy['minimum_horizon_samples']),
            ...
            results.extend({'gate': gate, 'horizon_hours': horizon, 'passed': bool(passed), 'actual': actual, 'required': required} for gate, passed, actual, required in checks)
        return results

    @staticmethod
    def _gates_pass(results): return bool(results) and all(x['passed'] for x in results)
    ...
    def _sample_fingerprint(rows):
        payload = [(int(x['id']), str(x.get('direction') or ''), int(x.get('horizon_hours') or 0),
                    str(x.get('actual_return_pct') if x.get('actual_return_pct') is not None else ''), str(x.get('features_json') or '{}')) for x in rows]
    ...
    def _metrics(self, shadow):
        ...
        for horizon, items in sorted(grouped.items()):
            n = len(items); active_returns = [x[3]['active_return_after_costs_pct'] for x in items]; candidate_returns = [x[3]['candidate_return_after_costs_pct'] for x in items]
            def drawdown(values):
    ...
    def _candidate(self, family, params, rows):
        ...
        best=dict(params); best_eval=self._score_parameter_set(best,rows); evaluated=1
        for _ in range(8):
            improved=False
            for name,step in steps.items():
                for direction in (-1,1):
                    trial=dict(best);lo,hi=BOUNDS[family][name];trial[name]=round(max(lo,min(hi,float(best[name])+step*direction)),4);ev=self._score_parameter_set(trial,rows);evaluated+=1
                    if ev['objective']>best_eval['objective']+1e-12: best,best_eval,improved=trial,ev,True
            if not improved: break
        self._last_search_details={'algorithm':'coordinate_search_v58','evaluated':evaluated,'training_count':len(rows),'best':best_eval}
        return best

    def propose(self, family, min_sample=10, min_improvement=.02):
        if family not in FAMILIES:return {'status':'UNKNOWN_FAMILY'}
        rows=self._evaluations(family); total=len(rows)
        if total<min_sample:return {'status':'INSUFFICIENT_DATA','sample_count':total,'required':min_sample}
        active=self.active(family)
        if not active:return {'status':'NO_ACTIVE_VERSION'}
        policy=self.gate_policy(); minimum_validation=max(3,int(self.db.value('learning_min_validation_samples','5')),len(policy['required_horizons'])*int(policy['minimum_horizon_samples']))
        validation_count=max(minimum_validation,int(math.ceil(total*.30)))
        if validation_count>=total:return {'status':'INSUFFICIENT_TRAINING','sample_count':total,'required_validation':minimum_validation,'training_count':max(0,total-validation_count),'validation_count':validation_count}
        training_rows=rows[:-validation_count];validation_rows=rows[-validation_count:];active_params=json.loads(active['parameters_json']);candidate=self._candidate(family,active_params,training_rows)
        if candidate==active_params:return {'status':'NO_PARAMETER_CHANGE','sample_count':total,'training_count':len(training_rows),'validation_count':len(validation_rows),'base_version':active['version']}
        shadow=self._shadow(validation_rows,active_params,candidate);metrics=self._metrics(shadow)
        active_decisions=sum(x[3]['active_signal']!='HOLD' for x in shadow);candidate_decisions=sum(x[3]['candidate_signal']!='HOLD' for x in shadow)
        active_correct=sum(x[1] for x in shadow if x[3]['active_signal']!='HOLD');candidate_correct=sum(x[2] for x in shadow if x[3]['candidate_signal']!='HOLD')
        active_accuracy=self._wilson(active_correct,active_decisions)[0] if active_decisions else 0.0;candidate_accuracy=self._wilson(candidate_correct,candidate_decisions)[0] if candidate_decisions else 0.0
        improvement=candidate_accuracy-active_accuracy;gates=self._gate_results(metrics,improvement,min_improvement,policy);status='PENDING' if self._gates_pass(gates) else 'REJECTED_GATE'
        reason='Alle Freigabe-Gates erfüllt; ausdrückliche Freigabe erforderlich' if status=='PENDING' else 'Mindestens ein Freigabe-Gate wurde nicht erfüllt'; fingerprint=self._sample_fingerprint(validation_rows)
        with self.db.con() as c:
            cur=c.execute('INSERT INTO learning_candidates(created_at,family,status,base_version,sample_count,active_accuracy,candidate_accuracy,improvement,ci_low,ci_high,parameters_json,reason,decided_at,gate_policy_json,gate_results_json,validation_fingerprint) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(now(),family,status,active['version'],len(validation_rows),str(active_accuracy),str(candidate_accuracy),str(improvement),str(self._wilson(candidate_correct,candidate_decisions)[0]),str(self._wilson(candidate_correct,candidate_decisions)[1] if candidate_decisions else 0.0),json.dumps(candidate,sort_keys=True),reason,None if status=='PENDING' else now(),json.dumps(policy,sort_keys=True),json.dumps(gates,sort_keys=True),fingerprint)); candidate_id=cur.lastrowid
            c.executemany('INSERT INTO learning_shadow_results(created_at,candidate_id,forecast_id,active_correct,candidate_correct,details_json) VALUES(?,?,?,?,?,?)',[(now(),candidate_id,fid,a,cc,json.dumps(details,sort_keys=True)) for fid,a,cc,details in shadow])
            c.executemany('INSERT INTO learning_candidate_metrics(candidate_id,horizon_hours,sample_count,active_decisions,candidate_decisions,active_coverage,candidate_coverage,active_net_return,candidate_net_return,net_return_improvement,active_max_drawdown,candidate_max_drawdown,details_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',[(candidate_id,m['horizon_hours'],m['sample_count'],m['active_decisions'],m['candidate_decisions'],str(m['active_coverage']),str(m['candidate_coverage']),str(m['active_net_return']),str(m['candidate_net_return']),str(m['net_return_improvement']),str(m['active_max_drawdown']),str(m['candidate_max_drawdown']),json.dumps(m,sort_keys=True)) for m in metrics])
        self.db.audit('CONTROLLED_LEARNING_CANDIDATE',json.dumps({'candidate_id':candidate_id,'family':family,'status':status,'training_count':len(training_rows),'validation_count':len(validation_rows),'improvement':improvement,'validation_fingerprint':fingerprint,'gates':gates},sort_keys=True))
        return {'status':status,'candidate_id':candidate_id,'sample_count':len(validation_rows),'total_sample_count':total,'training_count':len(training_rows),'validation_count':len(validation_rows),'improvement':improvement,'ci':[self._wilson(candidate_correct,candidate_decisions)[0],self._wilson(candidate_correct,candidate_decisions)[1] if candidate_decisions else 0.0],'metrics':metrics,'gate_policy':policy,'gate_results':gates,'validation_fingerprint':fingerprint}
    ...
    def decide(self,candidate_id,action):
        ...
        try:params=json.loads(proposal['parameters_json'])
        except Exception:return {'status':'INVALID_PARAMETER_SET'}
        if set(params)!=set(FAMILIES[proposal['family']]):return {'status':'INVALID_PARAMETER_SET'}
        for name,value in params.items():
            lo,hi=BOUNDS[proposal['family']][name]
            if not lo<=float(value)<=hi:return {'status':'OUT_OF_BOUNDS','parameter':name}
        ids=[int(x['forecast_id']) for x in self.db.rows('SELECT forecast_id FROM learning_shadow_results WHERE candidate_id=? ORDER BY id',(candidate_id,))]
        all_rows=self._evaluations(proposal['family']);by_id={int(x['id']):x for x in all_rows};validation_rows=[by_id[x] for x in ids if x in by_id]
        if len(validation_rows)!=len(ids):return self._block_recheck(candidate_id,'VALIDATION_SAMPLE_MISSING')
        fingerprint=self._sample_fingerprint(validation_rows)
        if not proposal.get('validation_fingerprint') or fingerprint!=proposal['validation_fingerprint']:return self._block_recheck(candidate_id,'VALIDATION_SAMPLE_CHANGED',{'sample_unchanged':False})
        policy=self.gate_policy();active_params=json.loads(current['parameters_json']);shadow=self._shadow(validation_rows,active_params,params);metrics=self._metrics(shadow)
        stored_gates=json.loads(proposal.get('gate_results_json') or '[]');required_accuracy=float(next((x.get('required') for x in stored_gates if x.get('gate')=='ACCURACY_IMPROVEMENT'),.02));gates=self._gate_results(metrics,float(proposal['improvement']),required_accuracy,policy)
        if not self._gates_pass(gates):return self._block_recheck(candidate_id,'GATE_RECHECK_FAILED',{'gate_results':gates})
        ...
        with self.db.con() as c:
            ...
            c.execute('INSERT INTO parameter_family_versions(created_at,family,version,status,parameters_json,parent_version,source,reason) VALUES(?,?,?,?,?,?,?,?)',(now(),proposal['family'],new_version,'ACTIVE',proposal['parameters_json'],current['version'],f'APPROVED_CANDIDATE_{candidate_id}','Explizite Benutzerfreigabe nach erneuter identischer Schattenprüfung'))
            c.execute("UPDATE learning_candidates SET status='APPROVED',decided_at=?,gate_policy_json=?,gate_results_json=? WHERE id=?",(now(),json.dumps(policy,sort_keys=True),json.dumps(gates,sort_keys=True),candidate_id))
        self.db.audit('CONTROLLED_LEARNING_APPROVED',json.dumps({'candidate_id':candidate_id,'family':proposal['family'],'version':new_version,'validation_fingerprint':fingerprint,'gates':gates},sort_keys=True))
        return {'status':'APPROVED','version':new_version,'gate_results':gates}

    def _block_recheck(self,candidate_id,reason,extra=None):
        payload={'candidate_id':candidate_id,'reason':reason}; payload.update(extra or {})
        with self.db.con() as c:c.execute("UPDATE learning_candidates SET status='REJECTED_RECHECK',decided_at=?,reason=? WHERE id=?",(now(),'Freigabe bei erneuter Prüfung blockiert: '+reason,candidate_id))
    ...
    def rollback(self,family,target_version):
        ...
        with self.db.con() as c:
            ...
            c.execute('INSERT INTO parameter_family_versions(created_at,family,version,status,parameters_json,parent_version,source,reason) VALUES(?,?,?,?,?,?,?,?)',(now(),family,next_version,'ACTIVE',target[0]['parameters_json'],current['version'],f'ROLLBACK_TO_{target_version}','Vollständiger kontrollierter Rollback'))
    ...
    def candidates(self,family=None):
        if family is None:return self.db.rows('SELECT * FROM learning_candidates ORDER BY id DESC LIMIT 100')
    ...
    def metrics(self,candidate_id=None,family=None):
        if candidate_id is not None:return self.db.rows('SELECT * FROM learning_candidate_metrics WHERE candidate_id=? ORDER BY candidate_id DESC,horizon_hours',(candidate_id,))
        if family is not None:return self.db.rows('SELECT m.* FROM learning_candidate_metrics m JOIN learning_candidates c ON c.id=m.candidate_id WHERE c.family=? ORDER BY m.candidate_id DESC,m.horizon_hours',(family,))
    ...
    def versions(self,family=None):
        if family is None:return self.db.rows('SELECT * FROM parameter_family_versions ORDER BY family,version DESC')
