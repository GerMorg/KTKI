...
class NewsLearning:
    ...
    def _fingerprint(rows):
        payload = []
        for row in rows:
            payload.append((str(row['id']), str(row.get('observed_at') or ''), str(row.get('title') or ''),
                            str(row.get('summary') or ''), str(row.get('source_class') or ''),
                            json.dumps(row.get('teacher') or {}, sort_keys=True, ensure_ascii=False)))
    ...
    def _split(self, rows, validation_ratio=.30, minimum_validation=3):
        ...
        policy = {'kind': 'EXPANDING_TIME_SPLIT', 'validation_ratio': ratio,
                  'minimum_validation': int(minimum_validation), 'training_count': len(training),
                  'validation_count': len(validation), 'training_start_at': edge(training, True),
    ...
    def _evaluate(self, rows, params):
        if not rows:
            return {'loss': 1.0, 'agreement': 0.0, 'samples': 0}
    ...
    def _optimize(self, rows, active):
        ...
        steps = {'positive_weight': .1, 'negative_weight': .1, 'uncertainty_penalty': .05,
                 'primary_source_bonus': .05, 'issuer_source_bonus': .05, 'relevance_floor': .05,
                 'confidence_floor': .05, 'priced_in_penalty': .05, 'impact_weight': .1}
        ...
        for _ in range(8):
            ...
            for name, step in steps.items():
                for direction in (-1, 1):
                    ...
                    lo, hi = BOUNDS[name]
                    trial[name] = round(max(lo, min(hi, trial[name] + step * direction)), 4)
                    loss = self._evaluate(rows, trial)['loss']
                    evaluated += 1
                    if loss < best - 1e-12:
                        candidate, best, improved = trial, loss, True
            if not improved: break
        self._last_search_details = {'algorithm': 'coordinate_search_v58', 'evaluated': evaluated,
                                     'training_count': len(rows), 'best_loss': best}
        return candidate
    ...
    def _walk_forward(self, rows, params, active_params, windows=3, minimum_validation=3, minimum_improvement=.01):
        ...
        for index in range(windows):
            ...
            active = self._evaluate(validation, active_params)
            candidate = self._evaluate(validation, params)
    ...
    def propose(self, min_sample=10, min_improvement=.01, automatic=False, validation_ratio=.30, minimum_validation=3, walk_forward_windows=3, required_stable_windows=2):
        ...
        if n < min_sample:
            ...
            return {'status': 'INSUFFICIENT_DATA', 'sample_count': n, 'required': min_sample,
                    'missing': diagnostic['missing'], 'reason': diagnostic['status'], 'data_status': diagnostic}
        active = self.active()
        if not active:
        ...
        previous = self.db.rows('SELECT id,status,base_version FROM news_model_candidates WHERE sample_fingerprint=? AND base_version=? ORDER BY id DESC LIMIT 1', (fingerprint, active['version']))
        if automatic and previous:
            return {'status': 'UNCHANGED', 'candidate_id': previous[0]['id'], 'candidate_status': previous[0]['status'], 'sample_count': n}
        active_params = json.loads(active['parameters_json'])
        candidate = self._optimize(training, active_params)
        train_old = self._evaluate(training, active_params); train_new = self._evaluate(training, candidate)
        old = self._evaluate(validation, active_params); new = self._evaluate(validation, candidate)
        ...
        comparison = {'training': {'active': train_old, 'candidate': train_new}, 'validation': {'active': old, 'candidate': new},
                      'minimum_validation_improvement': min_improvement, 'automatic_comparison': bool(automatic),
                      'window_policy': policy, 'walk_forward': walk, 'required_stable_windows': required_stable}
        ...
        values = (now(), status, active['version'], n, fingerprint, str(old['loss']), str(new['loss']), str(improvement),
                  str(old['agreement']), str(new['agreement']), json.dumps(candidate, sort_keys=True), json.dumps(comparison, sort_keys=True),
                  reason, None if passed else now(), len(training), len(validation), policy['training_start_at'], policy['training_end_at'],
                  policy['validation_start_at'], policy['validation_end_at'], json.dumps(policy, sort_keys=True), json.dumps(walk, sort_keys=True),
                  walk.get('stable_window_count', 0), required_stable)
        with self.db.con() as c:
            cur = c.execute('INSERT INTO news_model_candidates(created_at,status,base_version,sample_count,sample_fingerprint,active_loss,candidate_loss,improvement,agreement_active,agreement_candidate,parameters_json,comparison_json,reason,decided_at,training_count,validation_count,training_start_at,training_end_at,validation_start_at,validation_end_at,window_policy_json,walk_forward_json,stable_window_count,required_stable_windows) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', values)
            candidate_id = cur.lastrowid
        self.db.audit('NEWS_MODEL_CANDIDATE', json.dumps({'candidate_id': candidate_id, 'status': status, 'sample_fingerprint': fingerprint}, sort_keys=True))
        return {'status': status, 'candidate_id': candidate_id, 'sample_count': n, 'training_count': len(training), 'validation_count': len(validation),
                'improvement': improvement, 'comparison': comparison, 'sample_fingerprint': fingerprint}
    ...
    def decide(self, candidate_id, action):
        ...
        if not active or int(active['version']) != int(item['base_version']):
            with self.db.con() as c:
                c.execute("UPDATE news_model_candidates SET status='STALE',decided_at=? WHERE id=?", (now(), candidate_id))
            return {'status': 'STALE'}
        if action == 'reject':
            with self.db.con() as c:
                c.execute("UPDATE news_model_candidates SET status='REJECTED',decided_at=? WHERE id=?", (now(), candidate_id))
        ...
        for key, value in params.items():
            ...
            if not lo <= float(value) <= hi:
                return {'status': 'OUT_OF_BOUNDS', 'parameter': key}
        rows = self._samples()
        fingerprint = self._fingerprint(rows)
        stored_policy = json.loads(item.get('window_policy_json') or '{}')
        training, validation, current_policy = self._split(rows, stored_policy.get('validation_ratio', .30), stored_policy.get('minimum_validation', 3))
        comparison = json.loads(item.get('comparison_json') or '{}')
        required = float(comparison.get('minimum_validation_improvement', .01))
        if fingerprint != item['sample_fingerprint']:
            return self._block(candidate_id, 'SAMPLE_CHANGED', {'sample_unchanged': False})
        active_params = json.loads(active['parameters_json'])
        old = self._evaluate(validation, active_params); check = self._evaluate(validation, params)
        ...
        walk = self._walk_forward(rows, params, active_params, stored_walk.get('window_count', 3), stored_policy.get('minimum_validation', 3), required)
        valid = (current_policy['no_overlap'] and old['loss'] - check['loss'] >= required and check['agreement'] >= old['agreement']
                 and walk['status'] == 'VALID' and walk['stable_window_count'] >= required_stable and params != active_params)
        if not valid:
            return self._block(candidate_id, 'GATE_RECHECK_FAILED', {'validation': {'active': old, 'candidate': check}, 'walk_forward': walk})
        version = int(active['version']) + 1
        with self.db.con() as c:
            c.execute("UPDATE news_model_versions SET status='SUPERSEDED' WHERE status='ACTIVE'")
            c.execute('INSERT INTO news_model_versions(created_at,version,status,parameters_json,parent_version,source,reason) VALUES(?,?,?,?,?,?,?)',
                      (now(), version, 'ACTIVE', item['parameters_json'], active['version'], f'APPROVED_CANDIDATE_{candidate_id}',
                       'Explizite Benutzerfreigabe nach identischer zeitlich getrennter Validierung'))
            c.execute("UPDATE news_model_candidates SET status='APPROVED',decided_at=? WHERE id=?", (now(), candidate_id))
        self.db.audit('NEWS_MODEL_APPROVED', json.dumps({'candidate_id': candidate_id, 'version': version, 'sample_fingerprint': fingerprint}))
    ...
    def _block(self, candidate_id, reason, extra=None):
        ...
        with self.db.con() as c:
            c.execute("UPDATE news_model_candidates SET status='REJECTED_RECHECK',decided_at=?,reason=? WHERE id=?",
