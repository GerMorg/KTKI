...
class MarketScanner:
 def __init__(self,db,client):self.db,self.client=db,client;self.lock=threading.Lock();self.ensure()
 def ensure(self):
 ...
 def run(self,symbols,interval=60,limit=None,delay_seconds=None):
  symbols=sorted(set(symbols));limit=max(1,min(int(limit or self.db.value('scanner_batch_size','10')),50));delay=max(0.0,min(float(delay_seconds if delay_seconds is not None else self.db.value('scanner_delay_seconds','1.05')),10.0))
  if not self.lock.acquire(blocking=False):
   self.db.audit('SCANNER_RUN_SKIPPED','Scannerlauf bereits aktiv','warning');return {'status':'BUSY','processed':0,'remaining':len(symbols)}
  try:
   cursor=int(self.db.value('scanner_cursor','0') or 0);cursor=cursor%len(symbols) if symbols else 0;batch=(symbols+symbols)[cursor:cursor+min(limit,len(symbols))] if symbols else [];next_cursor=(cursor+len(batch))%len(symbols) if symbols else 0;stamp=now();valid=buy=hold=avoid=0
   try:tickers=self.client.ticker(batch)
   ...
   for index,symbol in enumerate(batch):
    ...
    if delay and index<len(batch)-1:time.sleep(delay)
   self.db.set_setting('scanner_cursor',next_cursor);remaining=max(0,len(symbols)-len(batch));quality='VALID' if batch and valid==len(batch) else ('EMPTY' if not batch else 'INCOMPLETE')
   with self.db.con() as c:c.execute('INSERT INTO scanner_runs(created_at,symbols_requested,symbols_valid,buy_count,hold_count,avoid_count,quality) VALUES(?,?,?,?,?,?,?)',(stamp,len(batch),valid,buy,hold,avoid,quality))
   details={'market_total':len(symbols),'batch_start':cursor,'processed':len(batch),'next_cursor':next_cursor,'remaining_in_rotation':remaining,'valid':valid,'buy':buy,'hold':hold,'avoid':avoid,'quality':quality};self.db.audit('SCANNER_BATCH_RUN',json.dumps(details,ensure_ascii=False));return details
