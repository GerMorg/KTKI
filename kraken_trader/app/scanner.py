...
class MarketScanner:
 ...
 def run(self,symbols,interval=60,limit=None,delay_seconds=None):
  ...
  for symbol in symbols:
   try:ac=self.db.rows('SELECT asset_class FROM market_universe WHERE symbol=? LIMIT 1',(symbol,))
   except Exception:ac=[]
   groups.setdefault(ac[0]['asset_class'] if ac else 'currency',[]).append(symbol)
  for ac,batch in groups.items():
   try:tickers.update(self.client.ticker(batch,ac))
   except Exception:
    for symbol in batch:
     try:tickers.update(self.client.ticker([symbol],ac))
