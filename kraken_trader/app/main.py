...
@app.get('/')
def index():
 portfolio=db.rows('SELECT total_eur,quality,created_at FROM portfolio_snapshots ORDER BY id DESC LIMIT 1');stream_state=stream.status();private_state=private_stream.status();market_rows=db.rows('SELECT received_at FROM live_prices ORDER BY received_at DESC LIMIT 1');private_rows=db.rows('SELECT received_at FROM private_balances ORDER BY received_at DESC LIMIT 1');stream_state['dashboard_state']='DATEN VERFÃœGBAR' if market_rows else ('DEAKTIVIERT' if not stream_state.get('configured_enabled') else stream_state.get('effective_state','NOCH KEINE DATEN'));private_state['dashboard_state']='KONTODATEN VERFÃœGBAR' if private_rows or portfolio else ('DEAKTIVIERT' if not private_state.get('configured_enabled') else private_state.get('effective_state','NOCH KEINE DATEN'));pending=db.rows("SELECT COUNT(*) n FROM learning_candidates WHERE status='PENDING'");latest_fee=fees.latest()
 return page("""<h1>Ãœbersicht</h1><p class=lead>Zentrale ArbeitsoberflÃ¤che fÃ¼r Daten, Analyse, Simulation und kontrollierte Lernfreigaben. Jeder Schritt bleibt nachvollziehbar; reale Orders sind technisch ausgeschlossen.</p><div class=grid><div class=card><h3>Portfolio</h3><div class=metric>{{portfolio[0].total_eur if portfolio else 'â†’'}} â‚¬</div><span class=pill>{{portfolio[0].quality if portfolio else 'Noch kein Snapshot'}}</span></div><div class=card><h3>Marktdaten</h3><div class=metric>{{stream.dashboard_state}}</div><small>Public WebSocket: {{stream.effective_state}}</small></div><div class=card><h3>Kontodaten</h3><div class=metric>{{private.dashboard_state}}</div><small>Read-only Private WebSocket: {{private.effective_state}}</small></div><div class=card><h3>Lernfreigaben</h3><div class=metric>{{pending[0].n if pending else 0}}</div><small>offene Kandidaten</small></div><div class=card><h3>GebÃ¼hrenprofil</h3><div class=metric>{{fee.status if fee else 'â†’'}}</div><small>{{fee.created_at if fee else 'Noch nicht abgerufen'}}</small></div></div><div class=card><h2>Empfohlener Ablauf</h2><div class=steps><div class=step><div><b>Daten verbinden und prÃ¼fen</b><br><span class=muted>API, Portfolio und DatenqualitÃ¤t kontrollieren.</span><br><a href="{{url_for('api_status')}}">API Ã¶ffnen</a> Â· <a href="{{url_for('data_quality')}}">DatenqualitÃ¤t</a></div></div><div class=step><div><b>Produkte und GebÃ¼hren aktualisieren</b><br><a href="{{url_for('products_page')}}">Produkte</a> Â· <a href="{{url_for('fees_page')}}">GebÃ¼hren</a></div></div><div class=step><div><b>Analyse und Simulation ausfÃ¼hren</b><br><a href="{{url_for('scanner_page')}}">Analyse</a> Â· <a href="{{url_for('paper')}}">Paper-Handel</a></div></div><div class=step><div><b>Lernkandidaten bewusst freigeben</b><br><span class=muted>Ohne ausdrÃ¼ckliche Freigabe Ã¤ndert sich kein aktiver Parameter.</span><br><a href="{{url_for('controlled_learning_page')}}">Kontrolliertes Lernen</a><a href="{{url_for('news_learning_page')}}">Nachrichten-Lernen</a></div></div></div></div>""",portfolio=portfolio,stream=stream_state,private=private_state,pending=pending,fee=latest_fee)

@app.get('/process')
def process_page():
 systems=[
  {'name':'Marktuniversum','role':'wÃ¤hlt verfÃ¼gbare Produkte','status':'Kernpfad'},
  ...
  {'name':'Prognosebewertung + Forex Shadow','role':'liefert Holdout-, Kosten- und QualitÃ¤tsnachweise','status':'QualitÃ¤tssicherung'},
  {'name':'Paper Engine','role':'simuliert Orders und Portfolioeffekte','status':'Vorstufe, nicht Live-AusfÃ¼hrung'},
  {'name':'GebÃ¼hrenprofil + Execution Costs','role':'liefert KostenschÃ¤tzungen','status':'Kernpfad Bewertung'},
  {'name':'Portfolio Allocator + Decision Matrix','role':'unterstÃ¼tzt Zielgewicht und Regeln','status':'Noch nicht automatisch an Live-Order gekoppelt'},
  {'name':'Backtests','role':'historische PrÃ¼fung','status':'QualitÃ¤tssicherung'},
  {'name':'RealTradeEngine + Kraken','role':'validiert oder Ã¼bermittelt die ausdrÃ¼cklich bestÃ¤tigte Order','status':'Live-AusfÃ¼hrung'},
  {'name':'Private WebSocket + Portfolio Sync','role':'liest Kontostand und AusfÃ¼hrungsereignisse zurÃ¼ck','status':'Nachkontrolle'},
  {'name':'Audit, Monitoring, Steuerinfo','role':'Nachweis, Warnungen und Auswertung','status':'Begleitend'}]
 return page("""<h1>Von der Information zur realen Order</h1><p class=lead>Diese Seite zeigt den tatsÃ¤chlich implementierten Datenweg und trennt automatische Analyse, bewusste Freigaben und Live-AusfÃ¼hrung.</p><div class=flow>
 ...
 ('Information erfassen','REST, Ã¶ffentliche Marktdaten, Produktdaten und optional Nachrichten werden eingelesen. Private Kontodaten bleiben davon getrennt.'),
 ('QualitÃ¤t und Kosten prÃ¼fen','DatenqualitÃ¤t, GebÃ¼hrenprofil, Spread, VolatilitÃ¤t und geschÃ¤tzte Roundtrip-Kosten werden geprÃ¼ft.'),
 ('Kandidaten reduzieren','Marktuniversum, Prefilter und Scanner bestimmen die analysierten Symbole und erzeugen Merkmale.'),
 ('Signal berechnen','Das aktive freigegebene Strategieprofil der Produktfamilie erzeugt BUY, HOLD oder AVOID. Nachrichtenmerkmale kÃ¶nnen einflieÃŸen, wenn der Nachrichtenpfad Daten liefert.'),
 ...
 ('Zuerst simulieren','Paper Engine und Kraken-Validierungsmodus prÃ¼fen den Ablauf ohne Live-Order. Backtests und Prognoseauswertung liefern zusÃ¤tzliche QualitÃ¤tsnachweise.'),
 ('Live-Auftrag vorbereiten','Symbol, Seite, Typ, Volumen und gegebenenfalls Limitpreis werden eingegeben. Allowlist sowie Volumen-, Auftragswert-, Market-Order- und Tageslimits werden geprÃ¼ft.'),
 ('Realhandel freigeben','Realhandel muss aktiviert, der Kill-Switch aufgehoben und die Freigabephrase bestÃ¤tigt sein. Das daraufhin erzeugte Token ist zeitlich begrenzt und nur einmal nutzbar.'),
 ('Order Ã¼bermitteln und Ã¼berwachen','RealTradeEngine ruft Krakens AddOrder-Endpunkt auf. Antwort, Audit-Ereignis und private AusfÃ¼hrungsdaten dienen der Nachkontrolle.')],systems=systems)
...
def api_status():
 ...
 return page('<h1>API</h1><div class=card><p>REST: <b>{{rest}}</b> Â· privater WebSocket: <b>{{ws}}</b></p><form method=post><button name=action value=rest>REST prÃ¼fen</button> <button name=action value=websocket>WebSocket-Berechtigung prÃ¼fen</button></form><p>{{msg}}</p><p class=muted>Ã–ffentliche WebSocket-Marktdaten benÃ¶tigen keinen API-SchlÃ¼ssel. FÃ¼r private KontokanÃ¤le wird die Kraken-Berechtigung "Access WebSockets API" benÃ¶tigt.</p></div><div class=card><h2>Ã–ffentlicher WebSocket-v2-Livestream</h2><p>Status: <b>{{stream.effective_state}}</b> Â· Kraken: {{stream.system_status or "â†’"}} Â· Symbole: {{stream.symbol_count}}</p><p>Letzte Nachricht: {{stream.last_message_at or "â†’"}}</p><table>{% for x in prices %}<tr><td>{{x.symbol}}</td><td>{{x.last}}</td><td>Bid {{x.bid or "â†’"}}</td><td>Ask {{x.ask or "â†’"}}</td><td>{{x.received_at}}</td></tr>{% endfor %}</table></div><div class=card><h2>Privater Read-only WebSocket v2</h2><p>Status: <b>{{private.effective_state}}</b> Â· Balances-Sequenz: {{private.sequences.get("balances","â†’")}} Â· Executions-Sequenz: {{private.sequences.get("executions","â†’")}}</p><p>Letzte Nachricht: {{private.last_message_at or "â†’"}} Â· Fehler: {{private.last_error or "â†’"}}</p><h3>Live-Balances</h3><table>{% for x in private_balances %}<tr><td>{{x.asset}}</td><td>{{x.balance}}</td><td>Seq {{x.sequence}}</td><td>{{x.received_at}}</td></tr>{% endfor %}</table><h3>Letzte AusfÃ¼hrungsereignisse</h3><table>{% for x in executions %}<tr><td>{{x.event_type}}</td><td>{{x.symbol or "â†’"}}</td><td>{{x.order_id or "â†’"}}</td><td>Seq {{x.sequence}}</td><td>{{x.received_at}}</td></tr>{% endfor %}</table></div>',rest=db.value('kraken_status'),ws=db.value('websocket_status'),msg=msg,stream=stream.status(),prices=db.rows('SELECT * FROM live_prices ORDER BY symbol'),private=private_stream.status(),private_balances=db.rows('SELECT * FROM private_balances ORDER BY asset'),executions=db.rows('SELECT event_type,order_id,symbol,sequence,received_at FROM private_execution_events ORDER BY received_at DESC LIMIT 50'))
def sync_portfolio():
...
def portfolio():
 ...
 return page('''<h1>Realportfolio</h1><div class=card><form method=post><button>Kraken vollstÃ¤ndig synchronisieren</button></form><p>{{msg}}</p><p class=muted>Nullpositionen bleiben als HISTORICAL_ZERO erhalten, wenn das Asset in der Ledger-Historie vorkam.</p><table><tr><th>Asset</th><th>Menge</th><th>EUR-Kurs</th><th>EUR-Wert</th><th>Status</th></tr>{% for x in rows %}<tr><td>{{x.display_name}} <small>{{x.asset}}</small></td><td>{{x.amount}}</td><td>{{x.eur_price or "â†’"}}</td><td>{{x.eur_value or "â†’"}}</td><td><span class=tag>{{x.classification}}</span></td></tr>{% endfor %}</table></div><div class=card><h2>Historie</h2><table><tr><th>Zeit</th><th>Gesamt EUR</th><th>QualitÃ¤t</th><th>Unbewertet</th></tr>{% for x in history %}<tr><td>{{x.created_at}}</td><td>{{x.total_eur}}</td><td>{{x.quality}}</td><td>{{x.unpriced_asset_count}}</td></tr>{% endfor %}</table></div>''',rows=rows,history=history,msg=msg)
@app.get('/scanner')
def scanner_page():
 ...
 return page(render_template_string("""<h1>Research-Scanner</h1><p class=muted>Globale Nachrichten und PrimÃ¤rquellen â†’Â â€™ Taxonomie â†’Â â€™ Vorfilter â†’Â â€™ versionierte Watchlist â†’Â â€™ Detailanalyse â†’Â â€™ Prognosevergleich. Nachrichten sind kein Handelssignal.</p><form method=post action="{{url_for('run_scanner')}}"><button>Research-Pipeline starten</button></form>{% if job %}<div class=card><b>Job #{{job.id}} {{job.status}}</b> Â· {{job.stage}} Â· {{job.progress_current}}/{{job.progress_total}}<p class=bad>{{job.error or ''}}</p></div>{% endif %}<h2>Quellen</h2><table>{% for x in sources %}<tr><td>{{x.name}}</td><td>{{x.source_class}}</td><td>{{x.last_status or 'offen'}}</td><td>{{x.last_error or 'â†’'}}</td><td>{{x.last_checked_at or 'â†’'}}</td></tr>{% endfor %}</table><h2>Watchlist</h2><table><tr><th>Symbol</th><th>Kategorie</th><th>Vorfilter</th><th>Status</th><th>Detailscore</th><th>Signal</th></tr>{% for x in rows %}<tr><td>{{x.symbol}}</td><td>{{x.category}}</td><td>{{x.prefilter_score}}</td><td>{{x.status}}</td><td>{{x.score or 'â†’'}}</td><td>{{x.signal or 'â†’'}}</td></tr>{% endfor %}</table><h2>Versionen und Prognosen</h2><p>Ausgewertet: {{stats.total if stats else 0}} Â· Richtung richtig: {{stats.correct if stats else 0}}</p><table>{% for x in versions %}<tr><td>#{{x.id}}</td><td>{{x.created_at}}</td><td>{{x.item_count}} Kandidaten</td><td>{{x.status}}</td></tr>{% endfor %}</table>""",rows=rows,job=job,sources=sources,versions=versions,stats=stats[0] if stats else None))
...
def paper():
 ...
 return page(render_template_string('''<h1>Musterdepot</h1><div class=grid><div class=card><h3>Gesamtwert</h3><b>â‚¬ {{total}}</b></div><div class=card><h3>Cash</h3><b>â‚¬ {{cash}}</b></div><div class=card><h3>Positionen</h3><b>â‚¬ {{pv}}</b></div><div class=card><h3>DatenqualitÃ¤t</h3><b>{{'VALID' if not missing else 'INCOMPLETE'}}</b></div></div><form method=post action="{{url_for('run_paper')}}"><p><button>Paper-Strategie jetzt ausfÃ¼hren</button></p></form><h2>Positionen</h2><table><tr><th>Symbol</th><th>Menge</th><th>Ëœ Kosten EUR</th></tr>{% for x in positions %}<tr><td>{{x.symbol}}</td><td>{{x.quantity}}</td><td>{{x.avg_cost_eur}}</td></tr>{% endfor %}</table><h2>Simulierte Trades</h2><table><tr><th>Zeit</th><th>Symbol</th><th>Seite</th><th>Menge</th><th>AusfÃ¼hrung</th><th>GebÃ¼hr</th><th>Slippage</th><th>Grund</th></tr>{% for x in trades %}<tr><td>{{x.created_at}}</td><td>{{x.symbol}}</td><td>{{x.side}}</td><td>{{x.quantity}}</td><td>{{x.execution_price}}</td><td>{{x.fee_eur}}</td><td>{{x.slippage_eur}}</td><td>{{x.reason}}</td></tr>{% endfor %}</table>''',cash=cash,pv=pv,total=total,missing=missing,positions=positions,trades=trades))
@app.post('/paper/run')
def run_paper():
 try:run_paper_cycle()
...
def paper_decisions():return page(render_template_string('''<h1>Paper-Entscheidungen</h1><p class=muted>Deterministische Baseline: BUY ab +1 % 24h, SELL ab -1,5 % 24h; nur freigegebene Produkte, nur bei LIVE-Daten und aktivierter Analyse-/Paper-Automatik.</p><table><tr><th>Zeit</th><th>Symbol</th><th>Aktion</th><th>Score</th><th>QualitÃ¤t</th><th>AusgefÃ¼hrt</th><th>BegrÃ¼ndung</th></tr>{% for x in r %}<tr><td>{{x.created_at}}</td><td>{{x.symbol}}</td><td>{{x.action}}</td><td>{{x.score}} %</td><td>{{x.data_quality}}</td><td>{{'ja' if x.executed else 'nein'}}</td><td>{{x.reason}}</td></tr>{% endfor %}</table>''',r=db.rows('SELECT * FROM paper_decisions ORDER BY id DESC LIMIT 500')))
...
def learning_page():
 ...
 return page(render_template_string('''<h1>Lernfreigaben</h1><div class=card><p>Die Strategie Ã¤ndert keine Parameter automatisch. Ein Vorschlag wird zunÃ¤chst angezeigt und erst mit deiner Freigabe als gemeinsame Version aktiviert.</p><form method=post><button name=action value=create>Neuen Vorschlag berechnen</button> {% if latest and latest.status=='PENDING' %}<button name=action value=approve>Alle neun Parameter mit einem Klick bestÃ¤tigen</button>{% endif %}</form><p>{{msg}}</p>{% if latest %}<p>Status: <b>{{latest.status}}</b> Â· Stichprobe: {{latest.sample_count}} Â· Trefferquote: {{latest.accuracy or 'â†’'}}</p>{% endif %}</div><table><tr><th>Parameter</th><th>Aktuell</th><th>Vorschlag</th><th>ZulÃ¤ssiger Bereich</th></tr>{% for x in rows %}<tr><td>{{x.label}}</td><td>{{x.current}}</td><td>{{x.proposed if x.proposed is not none else 'â†’'}}</td><td>{{x.minimum}} bis {{x.maximum}}</td></tr>{% endfor %}</table>''',latest=latest,rows=rows,msg=msg))
...
def news_learning_page():
 ...
 return page(render_template_string("""<h1>Nachrichten-AI und lokale Auswertung</h1><div class=card><p>Die externe Nachrichten-AI dient nur als Vergleichsinstanz. Neue lokale Parameter werden im Schattenmodus geprÃ¼ft und niemals automatisch aktiviert.</p><p>Aktive lokale Version: <b>v{{active.version}}</b></p><div class=grid><div><b>Nachrichten</b><div class=metric>{{data_status.news_items}}</div></div><div><b>GÃ¼ltige AI-Auswertungen</b><div class=metric>{{data_status.ai_valid}} / {{data_status.required}}</div></div><div><b>Noch erforderlich</b><div class=metric>{{data_status.missing}}</div></div><div><b>Nicht verarbeitet / ungÃ¼ltig</b><div class=metric>{{data_status.ai_unprocessed}} / {{data_status.ai_invalid}}</div></div></div>{% if data_status.ready %}<p class=ok>Die Datenbasis ist fÃ¼r einen Vergleich bereit.</p>{% elif data_status.status=='NO_NEWS_ITEMS' %}<p class=warning>Es sind noch keine Nachrichten vorhanden. Zuerst Nachrichten abrufen.</p>{% elif data_status.status=='NO_VALID_AI_RESULTS' %}<p class=warning>Es gibt noch keine gÃ¼ltigen AI-Auswertungen. AI-Konfiguration prÃ¼fen und anschlieÃŸend "AI auswerten" verwenden.</p>{% else %}<p class=warning>FÃ¼r den Vergleich fehlen noch {{data_status.missing}} gÃ¼ltige AI-Auswertungen.</p>{% endif %}<form method=post><button name=action value=analyze>AI auswerten</button> <button name=action value=compare {% if not data_status.ready %}disabled title="Mindestens {{data_status.required}} gÃ¼ltige AI-Auswertungen erforderlich"{% endif %}>Vergleich berechnen</button></form>{% if result %}<details open><summary>Ergebnis</summary><pre>{{result|tojson(indent=2)}}</pre></details>{% endif %}</div><h2>Kandidaten</h2>{% if candidates %}<table><tr><th>ID</th><th>Status</th><th>Gesamt</th><th>Training / Validierung</th><th>Stabile Fenster</th><th>Verbesserung</th><th>Freigabe</th></tr>{% for x in candidates %}<tr><td>#{{x.id}}</td><td>{{x.status}}</td><td>{{x.sample_count}}</td><td>{{x.training_count}} / {{x.validation_count}}</td><td>{{x.stable_window_count}} / {{x.required_stable_windows}}</td><td>{{x.improvement}}</td><td>{% if x.status=='PENDING' %}<form method=post><input type=hidden name=candidate_id value={{x.id}}><button name=action value=approve>Freigeben</button><button name=action value=reject>Ablehnen</button></form>{% endif %}</td></tr>{% endfor %}</table>{% else %}<div class=card>Noch keine Vergleichskandidaten vorhanden.</div>{% endif %}<h2>Lokale Versionen</h2><table>{% for x in versions %}<tr><td>v{{x.version}}</td><td>{{x.status}}</td><td>{{x.source}}</td><td><small>{{x.parameters_json}}</small></td></tr>{% endfor %}</table>""",active=active,candidates=candidates,versions=versions,result=result,data_status=data_status))

@app.get('/products')
def products_page():
 rows=product_view.rows();return page(render_template_string("""<h1>Kanonische Produkte</h1><p class=muted>Eine IdentitÃ¤t je Basiswert und Anlageklasse. Alternative AusfÃ¼hrungspaare bleiben sichtbar, wÃ¤hrend genau ein Paar ausgewÃ¤hlt wird.</p><table><tr><th>IdentitÃ¤t</th><th>Klasse</th><th>GewÃ¤hltes Paar</th><th>Alternativen</th><th>EUR-Kosten</th><th>USD-Kosten</th><th>Letzte Wahl</th><th>Grund</th><th>Position</th></tr>{% for x in rows %}<tr><td>{{x.canonical_id}}</td><td>{{x.asset_class}}</td><td>{{x.selected_symbol or 'â†’'}}</td><td>{{x.alternatives|join(', ')}}</td><td>{{x.eur_cost or 'â†’'}}</td><td>{{x.usd_cost or 'â†’'}}</td><td>{{x.updated_at}}</td><td>{{x.selection_reason}}</td><td>{% if x.position_symbol %}{{x.position_symbol}} / {{x.position_quantity}}{% else %}â†’{% endif %}</td></tr>{% endfor %}</table>""",rows=rows))
@app.get('/decision-matrix')
def decision_matrix_page():
 rows=decision_matrix.recent();return page(render_template_string("""<h1>Umschichtungs-Regelmatrix</h1><p class=muted>Jede Regel wird einzeln gespeichert. Die erste nicht erfÃ¼llte Regel ist der sichtbare Blockierungsgrund.</p><table><tr><th>Zeit</th><th>Produkt</th><th>Aktion</th><th>Regel</th><th>Status</th><th>BegrÃ¼ndung</th></tr>{% for x in rows %}<tr><td>{{x.created_at}}</td><td>{{x.canonical_id}}<br><small>{{x.symbol}}</small></td><td>{{x.action}}</td><td>{{x.rule_key}}</td><td class={{'good' if x.passed else 'bad'}}>{{'ERFÃœLLT' if x.passed else 'BLOCKIERT'}}</td><td>{{x.reason}}</td></tr>{% endfor %}</table>""",rows=rows))

@app.route('/forex-shadow',methods=['GET','POST'])
def forex_shadow_page():
 result=forex_shadow.run() if request.method=='POST' else None;rows=forex_shadow.comparisons();return page(render_template_string("""<h1>Forex v2 Schattenmodus</h1><div class=card><p><b>Keine Handelswirkung.</b> forex-v2 wird parallel zu forex-v1 ausgewertet. Alle EingÃ¤nge werden versioniert gespeichert. Nicht verfÃ¼gbare Makrofaktoren bleiben ausdrÃ¼cklich <code>null</code> und verbessern den Score nicht.</p><form method=post><button>Schattenbewertung ausfÃ¼hren</button></form><p>{{result or ''}}</p></div><table><tr><th>Zeit</th><th>Symbol</th><th>Aktiv</th><th>Kandidat</th><th>Abweichung</th></tr>{% for x in rows %}<tr><td>{{x.created_at}}</td><td>{{x.symbol}}</td><td>{{x.active_model}}: {{x.active_score}} / {{x.active_signal}}</td><td>{{x.candidate_model}}: {{x.candidate_score}} / {{x.candidate_signal}}</td><td>{{'ja' if x.disagrees else 'nein'}}</td></tr>{% endfor %}</table>""",rows=rows,result=result))
...
def fees_page():
 ...
 latest=fees.latest();rows=fees.rows();return page(render_template_string("""<h1>Kontospezifische GebÃ¼hren</h1><div class=card><p>Read-only Abruf der 30-Tage-HandelsaktivitÃ¤t und paarbezogenen Maker-/Taker-Stufen. Bei fehlender Berechtigung bleiben die konfigurierten konservativen GebÃ¼hren aktiv.</p><form method=post><button>GebÃ¼hrenprofil abrufen</button></form><p>{{result or ''}}</p>{% if latest %}<p>Status: <b>{{latest.status}}</b> Â· Quelle: {{latest.source}} Â· Volumen: {{latest.volume_30d or 'â†’'}} {{latest.volume_currency or ''}} Â· {{latest.created_at}}</p><p class=bad>{{latest.error_reason or ''}}</p>{% endif %}</div><table><tr><th>Symbol</th><th>Maker bps</th><th>Taker bps</th><th>Quelle</th><th>Zeitpunkt</th></tr>{% for x in rows %}<tr><td>{{x.symbol}}</td><td>{{x.maker_bps}}</td><td>{{x.taker_bps}}</td><td>{{x.source}}</td><td>{{x.effective_at}}</td></tr>{% endfor %}</table>""",result=result,latest=latest,rows=rows))

@app.get('/data-quality')
def data_quality():
 rows=history.diagnostics();return page(render_template_string("""<h1>DatenqualitÃ¤t</h1><p class=muted>Forex und andere MÃ¤rkte werden getrennt nach Ticker, Bid/Ask, Volumen, OHLC und Fehlergrund geprÃ¼ft. Eine laufende OHLC-Kerze zÃ¤hlt nicht als abgeschlossen.</p><table><tr><th>Symbol</th><th>Klasse</th><th>Ticker</th><th>Bid / Ask</th><th>Volumen</th><th>OHLC</th><th>Punkte</th><th>Fehler</th></tr>{% for x in rows %}<tr><td>{{x.symbol}}</td><td>{{x.asset_class}}</td><td>{{x.ticker_status}}<br><small>{{x.ticker_at or 'â†’'}}</small></td><td>{{x.bid or 'â†’'}} / {{x.ask or 'â†’'}}</td><td>{{x.volume or 'â†’'}}</td><td>{{x.ohlc_status}}<br><small>{{x.ohlc_at or 'â†’'}}</small></td><td>{{x.ohlc_points}}</td><td>{{x.error_reason or 'â†’'}}</td></tr>{% endfor %}</table>""",rows=rows))
