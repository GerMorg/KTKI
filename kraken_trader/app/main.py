...
def sync_pair_rules():
 pairs=client.pairs();stamp=__import__('db').now();saved=0
 with db.con() as c:
...
def run_paper_cycle():
 refresh_allowed_prices();sync_pair_rules();scanner.run(allowed_symbols(),60);configure_engine(paper_engine);return paper_engine.run()
def paper_scheduler():
 while True:
...
@app.get('/health')
def health():return {'status':'ok','version':'0.1.0-dev.9','real_trading':False,'websocket_status':db.value('websocket_status','not_checked'),'market_stream':stream.status(),'private_stream':private_stream.status()}
...
@app.get('/scanner')
def scanner_page():
 rows=db.rows('SELECT * FROM scanner_results ORDER BY CAST(score AS REAL) DESC');run=db.rows('SELECT * FROM scanner_runs ORDER BY id DESC LIMIT 1');return page(render_template_string('''<h1>Markt-Scanner</h1><p class=muted>Statistische Baseline auf abgeschlossenen 1-Stunden-Kerzen. Score kombiniert Momentum, SMA-Trend, VolatilitÃ¤t und aktuellen Spread.</p><form method=post action="{{url_for('run_scanner')}}"><button>Freigegebene Produkte jetzt scannen</button></form>{% if run %}<div class=card><b>Letzter Lauf: {{run.created_at}}</b> Â· {{run.symbols_valid}}/{{run.symbols_requested}} valide Â· BUY {{run.buy_count}} Â· HOLD {{run.hold_count}} Â· AVOID {{run.avoid_count}} Â· {{run.quality}}</div>{% endif %}<table><tr><th>Symbol</th><th>Score</th><th>Signal</th><th>Momentum</th><th>Trend</th><th>VolatilitÃ¤t</th><th>Spread</th><th>Daten</th><th>QualitÃ¤t</th><th>GrÃ¼nde</th></tr>{% for x in rows %}<tr><td>{{x.symbol}}</td><td>{{x.score}}</td><td><b>{{x.signal}}</b></td><td>{{x.momentum_pct or 'â€”'}} %</td><td>{{x.trend_pct or 'â€”'}} %</td><td>{{x.volatility_pct or 'â€”'}} %</td><td>{{x.spread_pct or 'â€”'}} %</td><td>{{x.data_points}}</td><td>{{x.quality}}</td><td>{{x.reasons_json}}</td></tr>{% endfor %}</table>''',rows=rows,run=run[0] if run else None))
...
def settings():
 if request.method=='POST':
  db.set_setting('automation_enabled','true' if request.form.get('automation') else 'false');db.set_setting('scanner_required','true' if request.form.get('scanner_required') else 'false');db.allow(request.form.getlist('products'))
