...
NAV_ITEMS=[('/', 'Ãœbersicht'),('/portfolio','Portfolio'),('/products','Produkte'),('/scanner','Analyse'),('/paper','Paper-Handel'),('/controlled-learning','Lernen'),('/news-learning','Nachrichten-Lernen'),('/fees','GebÃ¼hren'),('/data-quality','DatenqualitÃ¤t'),('/backtests','Backtests'),('/decision-matrix','Regelmatrix'),('/settings','Einstellungen'),('/audit','Audit'),('/exports','Export')]
def page(body,**ctx):
...
def controlled_learning_page():
 result=None;family=request.form.get('family') or request.args.get('family') or 'forex'
 if request.method=='POST':
  action=request.form.get('action')
 ...
 candidates=controlled_learning.candidates();versions=controlled_learning.versions();learning_metrics=controlled_learning.metrics();gate_policy=controlled_learning.gate_policy();active=controlled_learning.active(family);active_versions=controlled_learning.active_versions();active_params=json.loads(active['parameters_json']) if active else {};family_labels={'forex':'Forex','xstocks':'xStocks','crypto_spot':'Krypto Spot'}
 views=[]
...
def news_learning_page():
 ...
 if request.method=='POST':
  action=request.form.get('action')
  if action=='compare':result=news_learning.propose()
  elif action in ('approve','reject'):result=news_learning.decide(int(request.form.get('candidate_id')),action)
 active=news_learning.active();candidates=news_learning.candidates();versions=news_learning.versions()
 return page(render_template_string("""<h1>Nachrichten-AI und lokale Auswertung</h1><div class=card><p>Die externe Nachrichten-AI dient nur als Vergleichsinstanz. Neue lokale Parameter werden in mehreren zeitlich getrennten Walk-forward-Fenstern im Schattenmodus verglichen, aber niemals automatisch aktiviert.</p><p>Aktive lokale Version: <b>v{{active.version}}</b></p><form method=post><button name=action value=compare>Vergleich jetzt berechnen</button></form><p>{{result or ''}}</p></div><h2>Kandidaten</h2><table><tr><th>ID</th><th>Status</th><th>Gesamt</th><th>Training / Validierung</th><th>Validierungszeitraum</th><th>Stabile Fenster</th><th>Fehler aktiv / Kandidat</th><th>Verbesserung</th><th>RichtungsÃ¼bereinstimmung</th><th>Freigabe</th></tr>{% for x in candidates %}<tr><td>#{{x.id}}</td><td>{{x.status}}</td><td>{{x.sample_count}}</td><td>{{x.training_count}} / {{x.validation_count}}</td><td><small>{{x.validation_start_at or "â€“"}}<br>bis {{x.validation_end_at or "â€“"}}</small></td><td>{{x.stable_window_count}} / {{x.required_stable_windows}}</td><td>{{x.active_loss}} / {{x.candidate_loss}}</td><td>{{x.improvement}}</td><td>{{x.agreement_active}} / {{x.agreement_candidate}}</td><td>{% if x.status=='PENDING' %}<form method=post><input type=hidden name=candidate_id value={{x.id}}><button name=action value=approve>Lokale Version freigeben</button><button name=action value=reject>Ablehnen</button></form>{% endif %}</td></tr>{% endfor %}</table><h2>Lokale Versionen</h2><table>{% for x in versions %}<tr><td>v{{x.version}}</td><td>{{x.status}}</td><td>{{x.source}}</td><td><small>{{x.parameters_json}}</small></td></tr>{% endfor %}</table>""",active=active,candidates=candidates,versions=versions,result=result))
