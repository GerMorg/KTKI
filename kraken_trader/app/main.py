...
NAV_ITEMS=[('/', 'Übersicht'),('/portfolio','Portfolio'),('/products','Produkte'),('/scanner','Analyse'),('/news-ai','Nachrichten-AI'),('/paper','Paper-Handel'),('/controlled-learning','Lernen'),('/fees','Gebühren'),('/data-quality','Datenqualität'),('/backtests','Backtests'),('/decision-matrix','Regelmatrix'),('/settings','Einstellungen'),('/audit','Audit'),('/exports','Export')]
def page(body,**ctx):
...
def controlled_learning_page():
 ...
 labels={'forex':'Forex','xstocks':'xStocks','crypto_spot':'Krypto Spot'};families=[]
 for key,label in labels.items():
  active=controlled_learning.active(key);families.append({'key':key,'label':label,'active':active,'parameters':json.loads(active['parameters_json']) if active else {}})
 candidates=[]
 for item in controlled_learning.candidates():
  row=dict(item);row['parameters']=json.loads(row.get('parameters_json') or '{}');row['gates']=json.loads(row.get('gate_results_json') or '[]');row['gate_passed']=sum(1 for x in row['gates'] if x.get('passed'));row['gate_total']=len(row['gates']);candidates.append(row)
 return page("""<h1>Kontrolliertes Lernen</h1><p class=lead>Jede Produktart besitzt eine eigene Aktive Version und einen eigenen Parametersatz. Erst nach bestandenen Freigaberegeln und Ihrer ausdrücklichen Freigabe kann ein Kandidat aktiviert werden. Eine Freigabe ist nur bei Status PENDING möglich.</p>{% if result %}<div class=card><b>Ergebnis:</b> {{result}}</div>{% endif %}<h2>Aktive Parametersätze</h2><div class=grid>{% for f in families %}<div class=card><h3>{{f.label}}</h3><div class=metric>Version {{f.active.version if f.active else '—'}}</div><p class=muted>{{f.key}} ist der technische Familienname, nicht der einzige aktive Parametersatz.</p><details><summary>9 aktive Parameter anzeigen</summary><table>{% for k,v in f.parameters.items() %}<tr><td>{{k}}</td><td>{{v}}</td></tr>{% endfor %}</table></details><form method=post><input type=hidden name=family value="{{f.key}}"><button name=action value=propose>Kandidaten für {{f.label}} berechnen</button></form></div>{% endfor %}</div><h2>Kandidaten und Freigabe</h2><p class=muted>REJECTED_GATE bedeutet: mindestens ein Sicherheits-Gate wurde nicht bestanden. Dann gibt es absichtlich keinen Freigabeknopf. Nur PENDING kann freigegeben werden.</p><div class=tablewrap><table><tr><th>ID</th><th>Familie</th><th>Status</th><th>Gates</th><th>Verbesserung</th><th>Aktion</th></tr>{% for x in candidates %}<tr><td>#{{x.id}}</td><td>{{labels.get(x.family,x.family)}}</td><td><span class=pill>{{x.status}}</span></td><td>{{x.gate_passed}} / {{x.gate_total}}</td><td>{{x.improvement}}</td><td>{% if x.status=='PENDING' %}<form method=post><input type=hidden name=family value="{{x.family}}"><input type=hidden name=candidate_id value="{{x.id}}"><button name=action value=approve>Nach Gate-Neuprüfung freigeben</button> <button name=action value=reject>Ablehnen</button></form>{% else %}<span class=muted>Nicht freigabefähig</span>{% endif %}</td></tr>{% endfor %}</table></div>""",families=families,candidates=candidates,labels=labels,result=result)
...
@app.route('/news-ai',methods=['GET','POST'])
def news_ai_page():
 result=external_news_ai.analyze_pending() if request.method=='POST' else None
 rows=external_news_ai.results();calibration=external_news_ai.calibration()
 return page("""<h1>Externe Nachrichten-AI</h1><p class=lead>Optionale Schattenbewertung zur Prüfung des lokalen Nachrichtenmodells. Externe Ergebnisse ändern lokale Regeln niemals automatisch.</p><div class=grid><div class=card><h3>Konfiguration</h3><p>Provider: <b>{{provider}}</b><br>Modell: <b>{{model}}</b><br>Status: <b>{{'aktiv' if enabled else 'deaktiviert'}}</b></p><p class=muted>Für Google AI Studio: Provider google_ai_studio, API-Key und ein verfügbares Gemini-Modell eintragen.</p><form method=post><button>Ausstehende Nachrichten bewerten</button></form><p>{{result or ''}}</p></div><div class=card><h3>Sicherheitsgrenze</h3><p>Die externe AI ist nur Referenz. Abweichungen werden gespeichert, aber weder Taxonomie noch Strategieparameter automatisch verändert.</p></div></div><h2>Kalibriervergleich</h2><div class=tablewrap><table><tr><th>Nachricht</th><th>Provider / Modell</th><th>Lokale Themen</th><th>Externe Themen</th><th>Übereinstimmung</th><th>Prüfkandidaten</th></tr>{% for x in calibration %}<tr><td>{{x.title}}</td><td>{{x.provider}}<br><small>{{x.model}}</small></td><td>{{x.local_topics_json}}</td><td>{{x.external_topics_json}}</td><td>{{x.topic_overlap}}</td><td>{{x.missing_local_topics_json}}</td></tr>{% endfor %}</table></div><h2>Letzte externe Bewertungen</h2><div class=tablewrap><table><tr><th>Zeit</th><th>Nachricht</th><th>Status</th><th>Ergebnis / Fehler</th></tr>{% for x in rows %}<tr><td>{{x.created_at}}</td><td>{{x.title}}</td><td>{{x.status}}</td><td><small>{{x.error or x.result_json}}</small></td></tr>{% endfor %}</table></div>""",provider=opts.get('ai_provider','google_ai_studio'),model=opts.get('ai_model','gemini-2.5-flash'),enabled=bool(opts.get('ai_news_enabled') and opts.get('ai_api_key')),result=result,rows=rows,calibration=calibration)
