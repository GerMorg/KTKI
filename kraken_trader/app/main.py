...
def current_market_batch():
 symbols=allowed_symbols();size=max(1,min(int(float(db.value('scanner_batch_size','10'))),50));cursor=int(db.value('scanner_cursor','0') or 0);cursor=cursor%len(symbols) if symbols else 0
 return (symbols+symbols)[cursor:cursor+min(size,len(symbols))] if symbols else []
...
@app.get('/health')
def health():return {'status':'ok','version':'0.1.0-dev.11','real_trading':False,'websocket_status':db.value('websocket_status','not_checked'),'market_stream':stream.status(),'private_stream':private_stream.status()}
...
def settings():
 ...
 if request.method=='POST':
  ...
  rules={'paper_fee_bps':(0,500,40),'paper_slippage_bps':(0,500,10),'paper_max_position_pct':(1,100,10),'paper_trade_eur':(1,100000,25),'paper_interval_minutes':(1,1440,15),'scanner_batch_size':(1,50,10),'scanner_delay_seconds':(0.5,10,1.05)}
