...
def settings():
 if request.method=='POST':
  db.set_setting('automation_enabled','true' if request.form.get('automation') else 'false');db.set_setting('scanner_required','true' if request.form.get('scanner_required') else 'false');db.set_setting('research_auto_enabled','true' if request.form.get('research_auto_enabled') else 'false');universe.set_categories(set(request.form.getlist('categories')))
  rules={'paper_fee_bps':(0,500,40),'paper_slippage_bps':(0,500,10),'paper_max_position_pct':(1,100,10),'paper_trade_eur':(1,100000,25),'paper_interval_minutes':(1,1440,15),'scanner_delay_seconds':(.5,10,1.05),'prefilter_top_per_category':(1,25,8),'research_interval_minutes':(5,1440,60)}
