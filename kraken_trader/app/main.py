...
@app.get('/health')
def health():return {'status':'ok','version':'0.1.0-dev.6','real_trading':False,'websocket_status':db.value('websocket_status','not_checked'),'market_stream':stream.status(),'private_stream':private_stream.status()}
...
@app.post('/paper/run')
def run_paper():paper_engine.run();return redirect(url_for('paper_decisions'))
