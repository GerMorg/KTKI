...
def _is_payload_shape_error(exc):
    if not isinstance(exc, ValueError): return False
    text=str(exc).lower()
...
class ResearchPipeline:
 ...
 def _degrade(self,jid,stage,operation,exc,context=None):
  error=type(exc).__name__+': '+str(exc)[:500];self.db.audit('RESEARCH_STAGE_DEGRADED',json.dumps({'job_id':jid,'stage':stage,'operation':operation,'error':error,'context':context or {}},ensure_ascii=False,sort_keys=True,default=str),'warning');return {'status':'DEGRADED','error':error}
 ...
 def _recover_watchlist(self,limit=20):
  try:
   ...
   with self.db.con() as c:
    for symbol in symbols:
     row=c.execute('SELECT category FROM market_universe WHERE symbol=? ORDER BY category LIMIT 1',(symbol,)).fetchone();category=row[0] if row else 'crypto_spot'
  ...
  except Exception as exc:self.db.audit('RESEARCH_WATCHLIST_RECOVERY_FAILED',type(exc).__name__+': '+str(exc),'error');return []
