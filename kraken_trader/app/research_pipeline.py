...
class ResearchPipeline:
 ...
 def _run(self,jid):
  try:
   ...
   self.step(jid,'LEARNING_CANDIDATES',5)
   from controlled_learning import ControlledLearning
   from news_learning import NewsLearning
   learning={'controlled':ControlledLearning(self.db).propose_all(automatic=True),'news':NewsLearning(self.db).propose(automatic=True)}
   with self.db.con() as c:c.execute('UPDATE research_jobs SET status=?,stage=?,progress_current=?,finished_at=?,details_json=? WHERE id=?',('COMPLETED','DONE',6,now(),json.dumps({'universe':u,'prefilter':p,'scanner':s,'forex_shadow':shadow,'forecasts':forecast_count,'evaluated':evaluated,'learning':learning},ensure_ascii=False),jid))
