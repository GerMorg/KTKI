...
DEFAULTS={
 'automation_master_enabled':'false','automation_analysis_enabled':'false','automation_news_enabled':'false',
 'automation_learning_enabled':'false','automation_learning_auto_approve_enabled':'false',
 'automation_paper_enabled':'false','automation_real_enabled':'false','automation_real_execute_enabled':'false',
 'automation_tick_minutes':'5','automation_analysis_interval_minutes':'60','automation_news_interval_minutes':'30',
 'automation_learning_interval_minutes':'60','automation_paper_interval_minutes':'15','automation_real_interval_minutes':'60',
 'analysis_top_per_category':'5','analysis_max_symbols':'20','analysis_max_delay_seconds':'0.35',
...
class AutomationControllerV67:
 ...
 def ensure(self):
  with self.db.con() as c:c.executescript("CREATE TABLE IF NOT EXISTS automation_runs_v67(id INTEGER PRIMARY KEY AUTOINCREMENT,created_at TEXT NOT NULL,subsystem TEXT NOT NULL,status TEXT NOT NULL,automatic INTEGER NOT NULL,details_json TEXT NOT NULL,error TEXT);CREATE INDEX IF NOT EXISTS idx_automation_runs_v67_subsystem ON automation_runs_v67(subsystem,id DESC);")
 ...
 def enabled(self,subsystem):
  s=self.settings();return self.boolean(s['automation_master_enabled']) and self.boolean(s['automation_'+subsystem+'_enabled'])
 def due(self,subsystem,interval):
  rows=self.db.rows("SELECT created_at FROM automation_runs_v67 WHERE subsystem=? AND status IN ('COMPLETED','QUEUED') ORDER BY id DESC LIMIT 1",(subsystem,))
 ...
 def _record(self,subsystem,status,result=None,error=None):
  with self.db.con() as c:c.execute('INSERT INTO automation_runs_v67(created_at,subsystem,status,automatic,details_json,error) VALUES(?,?,?,?,?,?)',(now(),subsystem,status,1,json.dumps(result or {},sort_keys=True,ensure_ascii=False,default=str),error))
 def _sync(self,s):
 ...
 def run_once(self,force=False):
  with self.lock:
   ...
   self._sync(s);results={};items=[('news',30),('analysis',60),('learning',60),('paper',15),('real',60)]
   for subsystem,default_interval in items:
 ...
 def start_background(self):
  ...
  t=threading.Thread(target=self._loop,daemon=True,name='automation-v67');t.start();return t
 def _loop(self):
 ...
 def latest(self,limit=40):return self.db.rows('SELECT * FROM automation_runs_v67 ORDER BY id DESC LIMIT ?',(max(1,min(200,int(limit))),))
