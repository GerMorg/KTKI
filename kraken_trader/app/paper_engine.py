...
class PaperEngine:
 ...
 def execute(self,symbol,side,gross,reason,decision):
  ...
  if quote_usd:
   ...
   if not fx:raise ValueError('EUR/USD fehlt fÃƒÂ¼r FX-AusfÃƒÂ¼hrung')
  ...
  if side=='BUY':
   ...
   if qty<=0 or net>cash:raise ValueError('Nicht genÃƒÂ¼gend Paper-Cash')
 ...
 def stability_gate(self,symbol,action,improvement_after_costs=D(0)):
  ...
  return (matrix['allowed'],'StabilitÃƒÂ¤tsregeln erfÃƒÂ¼llt' if matrix['allowed'] else matrix['blocker'])
  ...
  for symbol,value in sorted(current.items(),key=lambda x:rank.get(x[0],D(0))):
   ...
   if active and value>=min_trade and gap>=edge_min+sell_hysteresis:
    ...
    if value*gap>costs:
     ...
     decision={'symbol':symbol,'confidence':str(conf),'better_confidence':str(best),'edge':str(gap),'estimated_roundtrip_cost_eur':str(costs),'leverage':1,'action':'SELL'};reason='Kostenbewusste Umschichtung: erwarteter Konfidenzvorteil ÃƒÂ¼ber Transferkosten'
