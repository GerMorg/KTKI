...
class Tests(unittest.TestCase):
 ...
 def test_utf8_source_has_no_mojibake(self):
  ...
  text=(Path(__file__).parents[1]/'app'/'main.py').read_text('utf-8');self.assertIn('ÃƒÅ“bersicht',text);self.assertNotIn('ÃƒÂ ',text)
 def test_html_declares_utf8(self):
  os.environ['APP_DISABLE_PAPER_SCHEDULER']='1';import main
  response=main.app.test_client().get('/');self.assertIn('charset=utf-8',response.headers['Content-Type'].lower());self.assertIn('ÃƒÅ“bersicht',response.data.decode('utf-8'))
