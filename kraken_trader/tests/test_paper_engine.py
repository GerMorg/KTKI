...
class PaperTests(unittest.TestCase):
 def setUp(self):
  self.db=DB(tempfile.mktemp());self.db.init(1000);self.db.set_setting('scanner_required','false');self.e=PaperEngine(self.db,1000,40,10,10,25)
  with self.db.con() as c:
   c.execute("INSERT OR REPLACE INTO live_prices VALUES(?,?,?,?,?,?)",('BTC/EUR','50000','49990','50010','2.0',now()))
   c.execute("INSERT OR REPLACE INTO allowlist VALUES('BTC/EUR',1)");c.execute("INSERT OR REPLACE INTO research_watchlist VALUES('BTC/EUR','crypto_spot','80','ANALYZED',?,1,'[]')",(now(),))
