...
class V68TaxLedgerTests(unittest.TestCase):
    ...
    def test_usd_trade_uses_cached_historical_fx_rate(self):
        ...
        fx = {'2025-05-01': Decimal('1.10'), '2025-05-05': Decimal('1.05')}
        report = self.service.analyze(2025, refresh=False)
        # Analyze() cannot invent FX; explicit rates are supplied to the pure builder.
        realized, inventory, warnings = self.service.build_realized(2025, fx)
        ...
        self.assertFalse(any('EUR/USD' in x for x in warnings))

    def test_missing_fx_is_review_required_not_false_precision(self):
        self.add_trade('u1', '2025-05-01T10:00:00', 'buy', 100, 1, 100, '1', 'BTC/USD')
    ...
    def test_missing_opening_basis_is_review_case(self):
        self.add_trade('s1', '2025-06-01T10:00:00', 'sell', 250, 1, 250, '2.5')
        realized, inventory, warnings = self.service.build_realized(2025, {})
    ...
    def test_persist_creates_hashed_machine_readable_exports(self):
        ...
        self.assertIn('e1kv', report['csv'])
        self.assertIn('realized', report['csv'])
        self.assertIn('inventory', report['csv'])
        self.assertIn('cashflow', report['csv'])
        self.assertIn('audit', report['csv'])
        package = self.service.export_zip(2025)
