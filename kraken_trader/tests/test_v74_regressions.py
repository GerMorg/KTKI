...
class V74RegressionTests(unittest.TestCase):
 ...
 def test_deep_scan_is_fail_soft_at_runtime_boundary(self):
  ...
  self.assertIn('_original_scanner_run = MarketScanner.run',source)
  self.assertIn('def _scanner_run_v74',source)
  self.assertIn("'DEEP_SCAN_DEGRADED'",source)
  self.assertIn("'status': 'DEGRADED'",source)
  self.assertIn('_original_shadow_run = ForexShadow.run',source)
  self.assertIn("'FOREX_SHADOW_DEGRADED'",source)
 def test_active_runtime_does_not_use_unprotected_ticker_update(self):
  source=(APP/'prefilter.py').read_text(encoding='utf-8')
  self.assertNotIn('tickers.update(self.client.ticker(block,ac))',source)
  self.assertNotIn('tickers[ac].update(single if isinstance(single,dict) else {})',source)
  ...
  for name in ('scanner.py','prefilter.py','execution_costs.py','execution_router.py','market_universe.py','payload_utils.py','strategy_profiles.py'):
   ...
   for node in ast.walk(tree):
    if isinstance(node,ast.For) and isinstance(node.target,ast.Tuple):
     value=node.iter
     if isinstance(value,ast.Name) and value.id in {'payload','response','result','raw','items'}:
      self.fail(f'unsafe tuple unpacking in {name}')
