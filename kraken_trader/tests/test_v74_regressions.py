...
class V74RegressionTests(unittest.TestCase):
 def test_active_runtime_and_version_are_v82(self):
  ...
  self.assertIn('v82_main:app',run);self.assertIn("APP_VERSION='0.1.0-dev.82'",version);self.assertIn('version: 0.1.0-dev.82',config);self.assertIn('version: 0.1.0-dev.82',repo);self.assertIn("'research_shape_error_quarantine': True",(APP/'v76_main.py').read_text(encoding='utf-8'));self.assertIn("'version':'0.1.0-dev.82'",runtime)
