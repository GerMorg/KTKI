...
class V74RegressionTests(unittest.TestCase):
 def test_active_runtime_and_version_are_v85(self):
  run=(ROOT/'run.sh').read_text(encoding='utf-8');version=(APP/'version.py').read_text(encoding='utf-8');config=(ROOT/'config.yaml').read_text(encoding='utf-8');runtime=(APP/'v85_main.py').read_text(encoding='utf-8');repo=(REPO/'repository.yaml').read_text(encoding='utf-8')
  self.assertIn('v85_main:app',run);self.assertIn("APP_VERSION='0.1.0-dev.85'",version);self.assertIn('version: 0.1.0-dev.85',config);self.assertIn('version: 0.1.0-dev.85',repo);self.assertIn("'research_shape_error_quarantine': True",(APP/'v76_main.py').read_text(encoding='utf-8'));self.assertIn('"version": "0.1.0-dev.85"',runtime)
