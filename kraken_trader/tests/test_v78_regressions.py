...
class V78RegressionTests(unittest.TestCase):
 ...
 def test_root_can_be_smoke_imported_without_network_feeds(self):
  old=os.environ.get('APP_DISABLE_WEBSOCKETS');old_data=os.environ.get('APP_DATA_DIR');os.environ['APP_DISABLE_WEBSOCKETS']='1';os.environ['APP_DATA_DIR']='/tmp/kraken-trader-v78-test'
  try:
   import importlib,sys;sys.modules.pop('v78_main',None);mod=importlib.import_module('v78_main');response=mod.app.test_client().get('/');self.assertEqual(response.status_code,200);self.assertIn(b'Kraken Trader',response.data)
  finally:
