...
class V78RegressionTests(unittest.TestCase):
 def test_v78_compatibility_runtime_remains_available(self):
  runtime=(APP/'v78_main.py').read_text(encoding='utf-8');self.assertIn("app.view_functions['index']=_dashboard",runtime);self.assertIn("@app.get('/v78-health')",runtime)
 ...
 def test_all_v78_runtime_sources_parse(self):
  for name in ('v78_main.py','v79_main.py','v77_main.py','v76_main.py','research_pipeline.py','market_universe.py','scanner.py','prefilter.py','ws_private.py','portfolio_sync.py'):ast.parse((APP/name).read_text(encoding='utf-8'),filename=name)
