...
class Tests(unittest.TestCase):
 def test_known_double_encoded_examples(self):
  ...
  pairs=[(bad(0xc3,0x153)+'bersicht','Ãœbersicht'),('Geb'+bad(0xc3,0xbc)+'hr','GebÃ¼hr'),(bad(0xe2,0x201a,0xac),'â‚¬'),(bad(0xe2,0x20ac,0x201d),'â€”'),('vollst'+bad(0xc3,0xa4)+'ndig','vollstÃ¤ndig')]
 ...
 def test_existing_database_text_is_repaired_once(self):
  ...
  first=repair_database(db);self.assertEqual(db.rows("SELECT details FROM audit WHERE event='TEST'")[0]['details'],'GebÃ¼hr und Ãœbersicht');self.assertGreater(first['changed'],0);self.assertEqual(repair_database(db)['status'],'ALREADY_DONE')
