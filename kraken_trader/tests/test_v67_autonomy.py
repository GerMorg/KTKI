...
class V67RepositoryTests(unittest.TestCase):
 ...
 def test_runtime_uses_latest_entrypoint(self):
  run=(ROOT/'run.sh').read_text(encoding='utf-8');self.assertIn('v84_main:app',run)
 def test_versions_and_new_controls(self):
  ...
  self.assertIn("APP_VERSION='0.1.0-dev.84'",version);self.assertIn('version: 0.1.0-dev.84',config)
