...
os.environ['APP_DATA_DIR']=tempfile.mkdtemp();sys.path.insert(0,os.path.join(os.path.dirname(__file__),'..','app'));import main
class T(unittest.TestCase):
 def setUp(self):self.c=main.app.test_client()
 def test_tabs(self):
  for p in ['/','/api','/portfolio','/paper','/audit','/settings','/exports']:self.assertEqual(self.c.get(p).status_code,200,p)
