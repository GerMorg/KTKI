...
from autonomous_portfolio_v65 import DecisionPolicy, Holding
...
class V65EndToEndTests(unittest.TestCase):
    ...
    def test_model_to_eur_twin_is_complete(self):
        r=simulate_end_to_end(self.outputs(),1000,[],1000,policy=self.policy())
    ...
    def test_model_to_usd_tracks_fx_and_trade_cost(self):
        r=simulate_end_to_end(self.outputs('USD'),1000,[],1000,fx_rate_eur_per_usd=.85,policy=self.policy())
        self.assertEqual(r['status'],'SIMULATED')
    ...
    def test_cheapest_route_is_selected(self):
        ...
        self.assertEqual(plan['actions'][0]['currency'],'EUR')
        self.assertEqual(plan['actions'][0]['selected_route'],'BTC/EUR')
