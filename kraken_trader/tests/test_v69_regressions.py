...
class V69RegressionTests(unittest.TestCase):
 def test_runtime_keeps_legacy_baselines_and_uses_latest_runtime(self):
  run=(ROOT/'run.sh').read_text(encoding='utf-8');self.assertIn('Historical runtimes remain preserved for compatibility',run);self.assertIn('v79_main:app',run);self.assertNotIn('v69_main:app',run);self.assertNotIn('v73_main:app',run);self.assertNotIn('v74_main:app',run);self.assertNotIn('v75_main:app',run);self.assertNotIn('v76_main:app',run);self.assertNotIn('v77_main:app',run);self.assertNotIn('v78_main:app',run)
