...
class V69RegressionTests(unittest.TestCase):
 def test_runtime_keeps_legacy_baselines_and_uses_latest_runtime(self):
  run=(ROOT/'run.sh').read_text(encoding='utf-8');self.assertIn('v69/v71/v72/v73/v74/v75 remain preserved compatibility baselines',run);self.assertIn('v76_main:app',run);self.assertNotIn('v69_main:app',run);self.assertNotIn('v73_main:app',run);self.assertNotIn('v74_main:app',run);self.assertNotIn('v75_main:app',run)
