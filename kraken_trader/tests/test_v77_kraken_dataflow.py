...
class V77KrakenDataflowTests(unittest.TestCase):
 ...
 def test_runtime_sources_compile(self):
  for path in (APP/'v77_main.py',APP/'v78_main.py',APP/'main.py',APP/'ws_private.py',APP/'portfolio_sync.py'):ast.parse(path.read_text(encoding='utf-8'),filename=str(path))
