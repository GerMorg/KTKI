...
class V77KrakenDataflowTests(unittest.TestCase):
 def test_active_runtime_and_version(self):
  ...
  self.assertIn('v80_main:app',run);self.assertIn("APP_VERSION='0.1.0-dev.80'",version);self.assertIn('version: 0.1.0-dev.80',config)
 ...
 def test_runtime_sources_compile(self):
  for path in (APP/'v77_main.py',APP/'v79_main.py',APP/'v80_main.py',APP/'main.py',APP/'ws_private.py',APP/'portfolio_sync.py'):ast.parse(path.read_text(encoding='utf-8'),filename=str(path))
