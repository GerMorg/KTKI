...
## xStocks-Kandidaten in dev.18
Ein aktivierter und von Kraken gemeldeter xStocks-Markt wird nicht mehr allein wegen eines vorübergehend fehlenden Tickers aus der Watchlist verworfen. `VALID`-Märkte werden zuerst gereiht; verbleibende Plätze können `PENDING_TICKER`-Märkte erhalten. Der Detail-Scanner versucht diese anschließend mit der richtigen Assetklasse. Ohne valide Detailanalyse bleibt jede Paper-Ausführung gesperrt.
